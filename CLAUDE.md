...
```bash
# Toolchain pinned in rust-toolchain.toml (Rust 1.95).
cargo test  -p graphrefly        # the clean-slate crate (the only workspace member)
cargo build -p graphrefly
cargo clippy -p graphrefly --all-targets
cargo fmt --all
