...
3. **Per-partition `parking_lot::ReentrantMutex` for the wave engine.** Mirrors the graphrefly-py per-subgraph RLock pattern (with split-eager union-find on top — `crates/graphrefly-core/src/subgraph.rs`). Two threads emitting on disjoint partitions run truly parallel for fn-fire-heavy waves (~1.23× at 2 threads per Phase J bench, 2026-05-09); same-partition emits serialize via the partition `wave_owner`. Cross-partition cascades acquire all touched partitions upfront in ascending `SubgraphId` order (Q7 from `archive/docs/SESSION-rust-port-d3-per-subgraph-parallelism.md`) — deadlock-free under any interleaving (verified by `wave_protocol_partitioned_MC` TLA+ harness in graphrefly-ts). **Caveat:** tight state-emit waves (no fn-fire) still serialize on the Core-global state mutex (Regime A); wide-spectrum parallelism for that regime awaits the Q2+Q3+Q-beyond per-partition state-shard refactor — see `docs/porting-deferred.md`.
