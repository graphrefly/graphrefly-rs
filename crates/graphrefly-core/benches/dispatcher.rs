...
fn build_chain(core: &Core, length: usize) -> (NodeId, NodeId) {
    ...
    let s = core.register_state(HandleId::new(1));
    let mut prev = s;
    let dummy_fn = FnId::new(0);
    for _ in 0..length {
        prev = core.register_derived(&[prev], dummy_fn, EqualsMode::Identity);
    }
    (s, prev)
}
...
fn build_diamond(core: &Core, fanout: usize) -> (NodeId, NodeId) {
    ...
    let inner: Vec<NodeId> = (0..fanout)
        .map(|_| core.register_derived(&[s], dummy_fn, EqualsMode::Identity))
        .collect();
    let sink = core.register_derived(&inner, dummy_fn, EqualsMode::Identity);
    (s, sink)
}
