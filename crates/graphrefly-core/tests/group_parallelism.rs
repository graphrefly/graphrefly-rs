...
//! correctness guarantees the group model must still uphold are:
//!
//! - Independent same-group Cores driven concurrently each serialize
//!   their own wave (in-order, exactly-once delivery) and never deadlock
//!   (the `join()` returning is the no-hang proof).
//! - Independent disjoint-group Cores do not block each other.
...
use graphrefly_core::{NodeFnOrOp, NodeOpts, NodeRegistration, SchedulingGroupId};
...
/// Build a `state → derived` cascade where BOTH nodes are declared into
/// scheduling group `g` (uniformly all-`Some` ⇒ component-consistent),
/// subscribe a recorder on the derived tail, then emit `1..=EMITS` and
/// return the values the tail delivered. Runs entirely on the calling
/// thread's own `Core`.
fn run_grouped_cascade(rt: &TestRuntime, g: SchedulingGroupId) -> Vec<TestValue> {
    ...
    let d = rt
        ...
        .expect("register grouped derived");
...
/// Independent disjoint-group Cores run concurrently and both complete
/// — they share no Core and no lock, so neither blocks the other.
/// (Replaces the deleted `disjoint_groups_cross_thread_waves_both_complete`.)
#[test]
fn disjoint_groups_independent_cores_both_complete() {
    let a = thread::spawn(|| {
        let rt = TestRuntime::new();
        run_grouped_cascade(&rt, GA)
    });
    let b = thread::spawn(|| {
        let rt = TestRuntime::new();
        run_grouped_cascade(&rt, GB)
    });
    let expected: Vec<TestValue> = std::iter::once(TestValue::Int(0))
        .chain((1..=EMITS).map(TestValue::Int))
        .collect();
    let av = a.join().expect("group-GA Core completed");
    let bv = b.join().expect("group-GB Core completed");
    assert_eq!(av, expected, "disjoint group GA Core delivered fully");
    assert_eq!(bv, expected, "disjoint group GB Core delivered fully");
}
