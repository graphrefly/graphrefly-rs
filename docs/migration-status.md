...
**2026-05-26 — D298 + D299 + D300 + D301 LANDED (`/porting-to-rs` Stage 2/3 per-Q slices on the post-Stage-1-sweep baseline).** All 4 Q-lock impls landed as per-Q graphrefly-rs commits + a single paired graphrefly-ts decision-log + cross-track-ledger commit. Pre-lock premise checks (per `feedback_verify_premise_before_greenfield`) caught 3 real gaps before code (semantic divergence for D298, D279-already-landed for D299, 3-round TS-has-no-NodeId discovery for D301).

- **D298 (Q1)** — `crates/graphrefly-graph/src/observe.rs` `GraphObserveOne::up(&self, core, messages) -> Result<(), UpError>` added alongside the existing typed `pause/resume/invalidate`. Two coexisting shapes with DIFFERENT semantics: `up(messages)` = canonical R3.6.2 upstream-injection via `Core::up` (matches TS pure-ts `Node.up(messages)` at `packages/pure-ts/src/core/node.ts:1430`); typed methods = direct mutation of the observed node. NOT sugar wrappers; same handle exposes both. 7 cargo regression tests pin the canonical semantic + tier-rejection + leaf-no-op + short-circuit. Override of D196 + `feedback_no_imperative` per verbatim `/porting-to-rs clear all... regardless if they need a valid consumer demand` directive. Commit `e9d1e7a`. Native binding + Impl widening + cross-arm parity scenario deferred per per-Q follow-on (cross-track-ledger §1 D298 row ⚠ OPEN with lift triggers).

- **D299 (Q2)** — VERIFIED NO-OP. D279 (2026-05-22, E-ii.3) already landed the sentinel-companion shape per `crates/graphrefly-graph/src/describe.rs:98-114` + `:75-97` rustdoc. Phase 1 premise verification (subagent 2 item 4) MISSED the D279 closure already documented in the rustdoc — calibration class same as the user-flagged "stale-premise propagation" anti-pattern. Folded into Stage 1 sweep as 23rd RESOLVED closure (`docs/porting-deferred.md` `DescribeValue::Rendered(Value::Null)` entry struck through). No code change. Folded into commit `a89e519`.

- **D300 (Q3)** — `crates/graphrefly-core/src/node.rs` `Core::set_deps` body: capture `old_topo_rank` BEFORE `rec.topo_rank` assignment; new BFS cascade block "2b" (lines 5174-5226) walks `s.children` recomputing each visited downstream consumer's `topo_rank`. Early-exit when rank unchanged. New `pub fn Core::topo_rank_of(node_id) -> Option<u32>` accessor for tests + consumer introspection. 3 cargo regression tests in `crates/graphrefly-core/tests/setdeps.rs` pin the cascade (`d300_set_deps_cascades_topo_rank_to_downstream_consumers`, `d300_set_deps_no_cascade_when_root_rank_unchanged`, `d300_cascade_stops_at_consumer_whose_other_dep_dominates`). Override of D196 + the prior Slice U "lift if hot path" defer-rationale. Substrate-internal; no cross-track-ledger row, no `Impl` widening. Commit `a89e519`.

- **D301 (Q4)** — three sub-decisions in one slice (B + B.a + B.b share render-site work per the per-Q slice plan). **B** converges Rust describe-output rendering for unnamed deps from `_anon_<NodeId>` to empty string `""` matching TS pure-ts (`packages/pure-ts/src/core/meta.ts:257`). TWO render sites converge together: `describe.rs:229` + `graph.rs:1055`. **B.a** drops the vestigial `NameError::ReservedPrefix` variant + `_anon_` prefix checks in `validate_name`/`is_valid_name` + collapses the `From<NameError> for MountError` pattern (value #14 vestigial-cleanup precedent — D253/D266/D267 lineage). **B.b** keeps the `_anon_<rawid>` marker in snapshot encode (`snapshot.rs:310`) with new persistence-vs-presentation rustdoc — verified during pre-lock check that `SnapshotError::UnresolvableDeps` Debug-format (`snapshot.rs:84` — `"unresolvable deps for node `{0}` (deps: {1:?})"`) preserves the marker verbatim through `{1:?}`. 4 caveats folded into the D301 lock text + rustdoc. Tests renamed + assertions rewritten: `tests/describe.rs::unnamed_dep_surfaces_as_empty_string_per_d301` (pins BOTH render sites in one test per caveat #1) + `tests/gap_fills.rs::edges_cross_graph_dep_shows_as_empty_string_per_d301`. Override of D196 + the prior "v1 acceptable" deferral. Commit `9267779`. Cross-track-ledger §1 D301 row added; TS pure-ts already conformant — no TS-side substrate change needed; cross-arm parity scenario authoring deferred per per-Q follow-on.

**Paired graphrefly-ts commit:** `ab00106` ("docs(decision-log): D298 + D299 + D300 + D301 ...") adds the 4 decision-log entries to `docs/rust-port-decisions.md` + the 2 new cross-track-ledger §1 rows (D298 + D301).

**Gate target after this batch:** 877/877 substrate (= 874 post-D297 + 3 new D300 setdeps tests; D298 adds 7 to graphrefly-graph 139→146 — graphrefly-graph is in `cargo nextest run` default-members but the per-crate test counts above were graphrefly-core only). Per-crate verified during the slice: graphrefly-core 308/308 + graphrefly-graph 146/146.

**Remaining open per-Q follow-ons (cross-track-ledger ⚠ OPEN rows):**
- D298 — native binding for `BenchObserveReactiveHandle::up(messages)` + wrapper.js exposure + Impl interface widening + cross-impl parity scenario. Lift trigger: a cross-arm consumer surfacing the need for `observe(path).up(messages)`.
- D301 — cross-impl parity scenario asserting BOTH `describe()` deps[] AND `edges()` for anon deps cross-arm (single scenario, both assertions). Lift trigger: a cross-arm consumer surfacing the parity contract OR a future cross-impl describe-shape audit pass.

**2026-05-26 — Stage 1 doc-closure sweep LANDED (`/porting-to-rs` `clear all the perf items and the deferred items. regardless if they need a valid consumer demand` directive — explicit override of D196 for this sweep).** Pre-implementation HALT premise check (per `feedback_verify_premise_before_greenfield`) found that of 196 `###` headers in `porting-deferred.md`, only ~50 represent genuinely-actionable open items after filtering archive sub-headers / FIXED-audit entries / narrative section headers. **22 RESOLVED-but-not-struck entries** were structurally closed by later decisions (D279 / Slice V2 / D241 / D246 / D253 / D255 actor model / D281 commit `602cd1b` / Slice F A6 / Slice U-napi S4 / D203 / D292 D.1) but never struck-through; **4 INTENTIONAL** entries (`pending_pause_overflow` panic-discard locked by D280, `Core::up` from fn-fire doc-only false-positive, `audit_of` racy narrow under D248/D255 single-owner, Unmount-vs-destroy cross-impl-convergent with no canonical mandate). **This sweep ships:** 26 total closures in `porting-deferred.md` (22 RESOLVED + 4 INTENTIONAL), unified single-style strikethrough with closing block + decision citation. No code changes; pure docs sweep. Gate unchanged 874/874 substrate (no test deltas). **Remaining actionable open work after sweep:** Stage 2 code fixes (F11 register Phase 3 double dep-walk, F17 typed-error UnknownNode surface widening, F3 reset_for_fresh_lifecycle lock-held, Graph::edges single-pass accumulator, topo_rank consumer cascade — Q3 user-locked Option 1, pending_notify mid-wave teardown ordering partial); Stage 3 spec calls (Q1 `GraphObserveOne::up()` user-locked Option 2 canonical-primary + typed sugar; Q2 `DescribeValue::Rendered(Value::Null)` user-locked Option 1 sentinel-companion alignment; Q4 anon-dep cross-arm render — re-pick pending after TS-no-NodeId discovery); Stage 4 explicit-deferred (D291 Items 2/3/3b pause/replay drain rollback, §10.5 wave-end arena bench-evidence-gated, S6 BenchReactiveMap.has/get async-cost evidence, F21 imbl persistent backends, F22 V1 CID versioning, F23 Graph-level sugar wrappers Rust-side, Per-partition nodes/children shards Q-beyond Sub-slice 4); post-1.0 / out-of-scope (M6 pyo3, BigInt migration, musl F10, --use-cross F13, CI TLA SHA pin); documented INTENTIONAL v1 limitations (GraphObserveAll subscribe accumulator, JS-callbacks-should-await, D022 Merge first-error, D023 Combine tuple-equality, WithLatestFrom semantic note, F15 hasFiredOnce approximation, F16 RustGraph mirror-update, RustNode.allocLockId, Error-payload non-i32, Option C async surface limitations, compact_every modulo, TEARDOWN tier exclusion, snapshot torn-read, etc.).

**2026-05-26 — D297 LANDED (`/porting-to-rs` Item 1 fix slice — bilateral convergence on pure-ts D282 `_preBatchSnapshot.teardownDone`).** Picked up via `/porting-to-rs next batch` continuation of option E (open `porting-deferred.md` items, D291 deferred-scope Items 1 + 3). Pre-implementation HALT premise checks (per `feedback_verify_premise_before_greenfield` + the calibrated D296 precedent) gave THREE findings: (i) **Item 1 inverted** — the deferral entry's "academic" framing was wrong; post-D291 a retry of `teardown(node)` after a panic-discarded batch was silently no-op'd by `teardown_inner`'s `has_received_teardown` idempotency guard (sits ABOVE the EmitTeardown arm; re-teardown short-circuits BEFORE touching `terminate_node`). Real R2.2.7.a + R2.6.4 + R4.3.2 invariant violation. (ii) **Item 3 framing held** — the cache snapshot covers the dominant case per R2.6.5 / Slice E1 lazy-fill. Late subscribers see `[START, DATA(restored_cache)]` rather than `[START, ...prior_buffer, DATA(cache)]`. Test-only pin would suffice (deferred per consumer-pressure gate). (iii) **NEW Item 3b surfaced** — `push_replay_buffer` mid-batch additions (`batch.rs:1944`) are NOT rolled back; subscribers alive across the rollback see handles emitted by a wave that didn't commit. R4.3.2 atomicity violation. Pure-ts handles this via the SAME `_preBatchSnapshot.replayBuffer` shallow content copy that closes Item 3. Bilateral convergence pre-lock check (user-locked option 4): **pure-ts has been conformant on BOTH Item 1 AND Items 3+3b since D282** — `_preBatchSnapshot` at [`packages/pure-ts/src/core/node.ts:3983`](../../graphrefly-ts/packages/pure-ts/src/core/node.ts) captures `teardownDone: this._teardownDone` (line 3993) AND `replayBuffer: [...this._replayBuffer]` (line 3999, /qa F5 "shallow content copy, not length") AND a pause sub-snapshot (line 4006, /qa F3). Rust catches up incrementally: D291 added terminal slots, D297 adds `has_received_teardown`, Items 2 + 3 + 3b stay deferred per consumer-pressure gates. **D297 ships:** new `wave_teardown_snapshots: AHashSet<NodeId>` field on `WaveState` (mirrors D291's `wave_terminal_snapshots` pattern); snapshot insert in `teardown_inner` `Action::Visit` (gated on `in_tick`); `Core::restore_wave_teardown_snapshots(s, ws) -> ()` sets `rec.has_received_teardown = false` for each snapshotted id on panic-discard; success-path drain extended in `drain_wave_terminal_snapshots`; outermost-wave-start `debug_assert`. 3 cargo regression tests in [`crates/graphrefly-core/tests/d297_teardown_flag_rollback.rs`](../crates/graphrefly-core/tests/d297_teardown_flag_rollback.rs) — retry-teardown-after-panic-succeeds (THE regression pin, asserts the second teardown delivers `[COMPLETE, TEARDOWN]` to a fresh subscriber; pre-fix was silently swallowed), success-path-preserves-idempotency-flag (R2.6.4 commit-path unchanged), cascade-children-teardown-flags-restore (cascade visits get snapshotted + restored as a group). **Cross-track-ledger §1 D282 row amended** with tightened conformance claim: "Rust arm IS conformant" → "conformant on cache + terminal slots (D291) + `has_received_teardown` (D297); replayBuffer + pauseState catch-up tracked under D291 deferred-scope Items 2 + 3 + 3b." `porting-deferred.md` D291 deferred-scope: Item 1 struck-through and closed; Item 3 framing corrected with R2.6.5 derivation + pure-ts citation; new Item 3b bullet added. Canonical: `~/src/graphrefly-ts/docs/rust-port-decisions.md` D297. Gate target: 874/874 substrate (= 871 post-D296 + 3 new D297 tests).

**2026-05-26 — D296 LANDED (`/porting-to-rs` test-only hardening slice).** Picked up under option E (open `porting-deferred.md` items, D291 deferred-scope Item 4 = `pending_wipes` queue rollback). Pre-implementation HALT premise check reframed the deferral entry's "D061-style leak class" framing: post-D291 the panic-discard's `mem::take` of `ws.pending_wipes` is the CORRECT behavior under R2.4.6 + R4.3.2, NOT a leak. Walking the path: (1) `restore_wave_terminal_snapshots` resets `rec.terminal = None` — the lifecycle did NOT end. (2) R2.4.6 says `wipe_ctx` fires "on resubscribable terminal RESET" — when a terminal lifecycle COMPLETES, not when one is attempted-and-aborted. (3) R4.3.2 atomicity (post-rollback observable state ≡ pre-batch observable state) requires the binding's `NodeCtxState` entry to SURVIVE rollback (the pre-batch lifecycle continues). Wiping on panic-discard would VIOLATE R4.3.2. Severity calibration: dormant on the napi `BenchBinding` (does not override `wipe_ctx`); latent for future bindings (PY/WASM/test bindings) that override `wipe_ctx`. **D296 ships:** doc-comment hardening at [`discard_wave_cleanup`'s `pending_wipes` drop site](../crates/graphrefly-core/src/batch.rs) citing R2.4.6 + R4.3.2 + 2 regression tests in [`crates/graphrefly-core/tests/d296_pending_wipes_rollback.rs`](../crates/graphrefly-core/tests/d296_pending_wipes_rollback.rs) pinning (a) panic-discard does NOT fire wipe + preserves NodeCtxState, (b) success path fires wipe exactly once. No WaveState widening, no behavior change. `porting-deferred.md` D291 deferred-scope Item 4 bullet reframed-and-struck-through. Canonical: `~/src/graphrefly-ts/docs/rust-port-decisions.md` D296. Gate (this slice): +2 cargo regression tests; the snapshot field below counts 871/871 substrate (= 869 + 2).

## Track-status snapshot — substrate-complete (2026-05-25)

**Where the Rust port sits today.** M1 lifecycle, M2 Slice F operator base, M3 (Slices E1/F/G/H + U/V/W + Slice 3e/3f cold sources + U-napi), M4 storage (A/B/C/D/E1/E2/F), and M5 reactive structures + bindings have all landed and are gate-GREEN. `@graphrefly/native` latest on npm = `0.0.3` (per-platform `.node` binaries for 5 targets, OIDC trusted publishing); bindings crate source is at `0.1.0` locally per D265 hold-local — D289/D290/D291 binding edits are awaiting the next user-gated tag push to republish. The D266–D270 cross-track-ledger closure batch + D271 storage cleanup + D272–D274 Arc→Rc / vestigial-surface sweep + D276–D281 storage-honest-error / perf / describe / `Core::up(Invalidate)` R1.4.2 convergence + D283–D287 `Impl.tagFactory` / `Impl.resourceProfile` paired-batch are all in. **2026-05-25 — D289 paired `/porting-to-rs` slice landed (binding-only, graphrefly-rs):** `BenchBatchContext` napi + parked-actor `BatchOp` loop + `Cell<bool>` tripwire (D288 Path D); +5 new cargo tests in `graphrefly-bindings-js` (first cargo tests in that crate). **2026-05-25 — D290 paired TS `/dev-dispatch` slice landed (graphrefly-ts + graphrefly-rs JS-only wrapper.js edit):** widened `Impl.batch(fn: (ctx: BatchCtx) => void)` parity contract + 3-line pure-ts ctx-injecting adapter + wrapper.js `impl.batch` (`createNativeImpl()` opens `state.core.openBatch()` + dispatches `ctx.down(node, msg)` by tier); 14+1 scenario rebase off `(src.inner as {down}).down(...)` → `ctx.down(src, msg)`; Cases 1/2/3/4/6/8/9/12 + new Case 15b run cross-arm. **2026-05-25 — D291 Bucket A `/porting-to-rs` slice landed (graphrefly-rs substrate + binding + CI hygiene):** Case 5 closed cross-arm; **Case 15a PARTIAL** — substrate Q2 tripwire ordering fix landed, but cross-arm vitest verification surfaced a separate libuv-sync-commit deadlock (sync napi blocks libuv → TSFN-backed JS sinks deadlock during `BatchGuard::Drop`'s success path), so Case 15a stays `runIf(impl.name === "pure-ts")` pending D292 async-commit slice. Item 1 (substrate, `graphrefly-core`): `wave_terminal_snapshots: AHashSet<NodeId>` + `wave_dep_terminal_snapshots: AHashSet<(NodeId, usize)>` on `WaveState`; `Core::terminate_node` snapshots on `None → Some(_)` transitions (entry + cascade) gated on `in_tick`; new `Core::restore_wave_terminal_snapshots(s, ws) -> Vec<HandleId>` releases ERROR retains lock-released; wired into `BatchGuard::discard_wave_cleanup` (R4.3.2 status-snapshot completeness for terminal slot + per-dep cascade slot). Item 2 (binding, `graphrefly-bindings-js`): explicit `drop(handle_guard)` BEFORE `drop(guard_holder.take())` in `BatchOp::Commit` arm of `run_batch_loop` so `DURING_BATCH_HANDLE` is clear when `BatchGuard::Drop`'s success path fires sinks via `fire_deferred`; rollback path's decl-order reverse-drop preserved (Q2 invariant on `discard_wave_cleanup` intact). Item 3 (Arc→Rc cleanup tail): **reframed — no work needed.** D272/D273 already swept the Cat-3 surface; empirical scan shows only 3 known-correct Cat-1/2 retains remain (`temporal.rs:1401`, `operators/tests/common/mod.rs:830`, `core/tests/lock_discipline.rs:198`), each with rationale comment. Item 4 (CI hygiene, D289 QA F2): `scripts/check-tripwire-wiring.sh` + `mise run check:tripwire-wiring` + wired into `gate.sh` Step 0 — fail-loud if any future `fn bridge_sync*` body in bindings-js omits `assert_no_batch_handle(`. Cross-track-ledger §1 D282 row PARTIAL closure — Case 5 cross-arm aligned; Case 15a row stays open pending D292 (do NOT archive yet). New tests: +3 substrate (`crates/graphrefly-core/tests/d291_terminal_rollback.rs`), +1 binding (`crates/graphrefly-bindings-js/src/batch_bindings.rs::tests::d291_commit_path_clears_during_batch_handle_before_sinks_fire`). Gate green at 869/869 default-members (+3 from 866) + 6/6 bindings-js (+1 from 5). Cross-arm parity full gate: 457/9 (+1 from Case 5 lift); batch-throw file: 27/7.
...
| D282 — `Impl.batch` shape | ⚠ PARTIAL (2026-05-25 — D288 + D289 + D290 + D291 Item 1); Case 15a stays open pending D292 async-commit | D288 (2026-05-24) locked Path D + Q1–Q5 sub-decisions. D289 (2026-05-25, native binding) shipped `BenchBatchContext` napi + parked-actor `BatchOp` loop + `Cell<bool>` tripwire + 5 cargo regressions. D290 (2026-05-25, paired TS `/dev-dispatch`) widened `Impl.batch(fn: (ctx: BatchCtx) => void)` + new `BatchCtx` type + 3-line pure-ts ctx-injecting adapter + wrapper.js `impl.batch` (JS-only, no Rust recompile) + 14+1 scenario rebase. **D291 (2026-05-25, Bucket A `/porting-to-rs`):** closes Case 5 cross-arm (terminal-status rollback via `wave_terminal_snapshots` + per-dep cascade snapshot + restore on panic-discard); Case 15a substrate Q2 tripwire-ordering fix landed BUT cross-arm vitest verification surfaced a separate libuv-sync-commit deadlock (sync napi blocks libuv → TSFN-backed JS sinks can't fire in `BatchGuard::Drop`'s success path). Case 15a stays `runIf(impl.name === "pure-ts")` pending D292 (queued async `BenchBatchContext::commit`/`rollback` slice). Cross-arm parity gate: 457/9 (full parity, +1 from Case 5 lift; full file run: 27/7 in `scenarios/core/batch-throw-rollback.test.ts`). |
...
- ~~**A** — Arc→Rc cleanup tail.~~ **CLOSED 2026-05-25 (D291 Item 3): verified no-op.** D272/D273 already swept the Cat-3 surface; empirical scan of `crates/` shows only 3 remaining `#[allow(clippy::arc_with_non_send_sync)]` annotations, all known-correct Cat-1/2 retains with rationale comments (`graphrefly-operators/src/temporal.rs:1401` — `em.defer(move |c| ...)` `Send+'static` bound; `graphrefly-operators/tests/common/mod.rs:830` — `Weak`-upgrade pattern; `graphrefly-core/tests/lock_discipline.rs:198` — test sink-closure capture across boundaries). Original "remaining sites" framing was stale pre-D273 text. No further work.
- **B** — M6 (Python/pyo3 bindings). Post-1.0 per migration plan.
- **C** — Deeper §10 perf simplifications (10.3 diamond bitmask + 10.5 wave-end notification arena + 10.6 sink iteration all validated NOT wins via D277 bench-evidence; 10.5 wave-end arena candidate remains pending a criterion bench reproducer that pays for itself).
- **✅ D293 — LANDED 2026-05-25 (`/porting-to-rs` patch slice; v0.0.8 candidate).** Closes the "process never exits" hang for every napi consumer (test frameworks, CLI scripts, serverless cold-start, AWS Lambda). Ships as `@graphrefly/native@0.0.8` (patch bump, additive only — no breaking surface). **Two pre-locked-scope shifts simplified the actual landing:** (1) `MailboxOp::Shutdown` variant NOT added — that enum lives in `graphrefly-core/src/mailbox.rs` for cross-thread producer/timer ops, not the `CoreActor` closure-dispatch channel; (2) `closed: AtomicBool` per-method guards NOT added — `actor.run`'s existing `sender.send(...).map_err(...)` branch surfaces the broadened "(actor is shut down or shutting down)" napi error automatically post-shutdown, so Q2a's clear-error contract is met without 15+ per-method guard sites.
  - **Substrate** (`crates/graphrefly-bindings-js/src/core_actor.rs`): new `shutdown_flag: Arc<AtomicBool>` field on `CoreActor`; worker loop gains `if worker_flag.load(Acquire) { break; }` at the top of each iteration; new `pub(crate) fn shutdown(&self)` (idempotent flag-swap + no-op wake closure + take-and-join `JoinHandle`); `Drop for CoreActor` simplifies to `self.shutdown()` + defensive sender-swap-and-drop safety net. Error message in `run` + `run_sync` broadened from "(actor shutdown raced with napi call)" to "(actor is shut down or shutting down)" per D293 /qa Q2a refinement (covers both Drop trigger AND explicit shutdown uniformly).
  - **Binding** (`crates/graphrefly-bindings-js/src/core_bindings.rs`): new `BenchCore::close()` async napi — drains subscriptions (existing dispose body, best-effort on error) then calls `self.actor.shutdown()`; `BenchCore::dispose()` becomes a thin async alias delegating to `close()` (Q1b — grep-verified safe: all 3 prior `dispose()` callers are test-end / lifecycle-end patterns, none do dispose-then-use).
  - **JS surface** (`crates/graphrefly-bindings-js/wrapper.js` + `wrapper.d.ts`): `impl.close()` public async method; `impl[Symbol.asyncDispose] = impl.close` so `await using impl = createNativeImpl()` works on Node 22+; `impl._dispose = impl.close` as parity-only alias. `wrapper.d.ts` typed with `close(): Promise<void>` + `[Symbol.asyncDispose](): Promise<void>` on `NativeImpl`.
  - **Parity** (`packages/parity-tests/impls/types.ts` + `pure-ts.ts`): `Impl.close(): Promise<void>` widened on the contract; pure-ts arm ships a no-op `async close()` (no actor thread to kill; symmetry only).
  - **Cargo regression** (`crates/graphrefly-bindings-js/src/batch_bindings.rs::tests::d293_shutdown_idempotent_and_post_close_errors_are_clear`): pins idempotency + post-shutdown error message shape + `JoinHandle` single-consumption invariant.
  - **Parity scenario** (`packages/parity-tests/scenarios/usage/cleanup.test.ts`): 5 cross-arm cases — close exists + resolves, double-close idempotent, post-close method calls reject (native) or stay live (pure-ts arm-conditional), `Symbol.asyncDispose` wired (native), implicit "vitest exits cleanly" Case 5 (the load-bearing regression for the whole D293 — if `close()` didn't kill the worker, vitest would hang at file end).
  - **README** (`crates/graphrefly-bindings-js/README.md`): NEW. "Closing a NativeImpl" section with modern (`await using`) + compat (`try/finally`) + test-framework + behavior-contract patterns; `process.exit()` escape-hatch sub-section; "what ships in v0.0.8 / what's coming in v0.1.0 (D292)".
  - **D292 watch items recorded** (one-line `// D292 WATCH:` comments at both code sites + this doc + porting-deferred):
    - **Drop-running-join on libuv thread** — GC of `BenchCore` on the libuv thread blocks the JS event loop on `handle.join()`. D292 should lock an async-shutdown-from-finalizer pattern (e.g., `FinalizationRegistry`-driven `napi::bindgen_prelude::spawn`).
    - **Silent close-error swallow in wrapper.js** — `impl.close = async () => { await state.core.close().catch(() => {}); }` preserves backward-compat with the prior `_dispose` shape but loses error visibility on the new public surface. D292 decides whether `impl.close()` rejects on actor errors or stays swallow-silent.
  - **Cross-track-ledger impact:** §1 row added 2026-05-25 (sweep paired with this slice, `~/src/graphrefly-ts/docs/cross-track-ledger.md`) — `Impl.close()` widening (cross-arm parity-coupled; both arms ship `close()` per D293). Row stays open until D292's v0.1.0 lifecycle slice lifts the two D292 WATCH items below; archives together with §1 D282 row + Case 15a on D292 close. `### D293` decision-log entry back-filled in `~/src/graphrefly-ts/docs/rust-port-decisions.md` in the same sweep.
  - **Gate:** `mise run gate` GREEN 869/869 (default-members substrate); `cargo nextest run -p graphrefly-bindings-js` 7/7 (includes the new D293 test); clippy + fmt clean; `#![forbid(unsafe_code)]` preserved.
  - **D293 cross-ref:** `docs/optimizations.md` D293 entry (re-frame from "🚢 SHIP FIRST" to "✅ LANDED"); memory `project_next_porting_batch.md` D293 block (mark landed); `~/src/graphrefly-rs/docs/porting-deferred.md` "process never exits" entry CLOSED.
  - **Local commit pending; not pushed** per D265 hold-local pattern (user-gated tag push triggers OIDC `@graphrefly/native@0.0.8` republish).

- **D — ✅ D292 LANDED 2026-05-25 (`/porting-to-rs` paired-batch slice = D295; design lock + implementation in same session).** Ships as `@graphrefly/native@0.1.0` post user-gated tag push (D265 hold-local). **Gate GREEN:** 869/869 substrate nextest --profile ci + 12/12 bindings-js (7 pre-existing + 5 new D292 regressions). Canonical: `~/src/graphrefly-ts/docs/rust-port-decisions.md` D292 (design lock) + D295 (implementation) + `~/src/graphrefly-ts/archive/docs/SESSION-DS-D292-native-async-lifecycle.md` (Q5–Q9 + R1–R5 + F1–F4 + Acceptance bar). Cross-track-ledger §1 D282 / D283 / D292 / D293 rows all CLOSED 2026-05-25 — archive together on next sweep post-republish.
  - **D.1 = A (locked) — async `BenchGraph::derived(name, deps, fn)` via D263 reroute.** 2 napi crossings (`registerUserDerived` + `addNode`); JS-side closure-cell map keyed by `fnId`; eviction wired into existing `nodesByName` teardown (`graph.remove` / `destroy` cascade). **R1 refinement:** `BenchGraph.derived` JSDoc cites anti-pattern #5 watch — if substrate teardown ever evolves outside `graph.remove`/`destroy`, update eviction wiring HERE. Lifts D287 carve-out on `scenarios/graph/resource-profile.test.ts` test #1 cross-arm.
  - **D.2 = A (locked) — preserve parked-actor; async `BenchBatchContext::commit`/`rollback` via `tokio::task::spawn_blocking(move || rx.recv())`.** **R2 refinement (D255 α-shape correction):** explicitly `spawn_blocking`, NOT `actor.run(|_| rx.recv())` — D255 single-worker-per-Core means a pending sync `rx.recv()` inside `actor.run` would serialize every concurrent actor.run read behind the pending commit. `spawn_blocking` moves the wait to tokio's blocking pool. **R3 refinement (rollback symmetry):** `BatchOp::Rollback` reply ALSO widens to `SyncSender<Result<(), String>>` + `catch_unwind` around `discard_wave_cleanup`, not just `BatchOp::Commit`'s `drop(guard_holder.take())`. Both arms symmetric. `closed: Cell<bool>` → `closed: AtomicBool` (cross-async-task safety; matches D293's `shutdown_flag` precedent). D291's explicit `drop(handle_guard)` BEFORE `drop(guard_holder.take())` ordering preserved (Q2 tripwire invariant). Lifts D291 Case 15a `runIf(pure-ts)` gate + closes BH15 sink-panic-hang.
  - **D.3 lifecycle residuals (5 sub-items locked):** Item 1 = A (FinalizationRegistry-driven `napi::bindgen_prelude::spawn(shutdown)` off-libuv async shutdown; `Drop for CoreActor` stays as non-libuv safety net); Item 2 = A (reject on actor errors — drop `.catch(() => {})` in `wrapper.js` `impl.close`); Item 3 = A (defer nested `Symbol.asyncDispose` on Graph/Subscription — D196 gate); Item 4 = A (close-waits/drain in-flight commit/rollback awaits; D288 Q3's `BatchContextInner::Drop` safety net bounds the drain by longest in-flight batch frame); Item 5 = B (default-off opt-in via `createNativeImpl({ autoCloseOnBeforeExit: true })`). **R4 refinement (Item 4):** "stuck closure → close hangs" warning moves from README-only to `Impl.close` JSDoc one-liner — users discover hangs at runtime, not in docs. **R5 refinement (Item 5):** README's "Closing a NativeImpl" section gains explicit "When to opt in to `autoCloseOnBeforeExit`" 3-condition rubric.
  - **Cross-cutting findings (F1–F4 locked):** F1 — close drains in-flight commit/rollback awaits (NOT "rollback unclosed contexts" — D288 Q3 makes truly-zombie ctxs impossible via `BatchContextInner::Drop` safety net); F2 — panic-propagation contract is shared across commit/rollback/close (every napi method MUST surface actor panics as rejected Promises with payload-string error message; wrapper.js MUST NOT `.catch(() => {})` actor errors); F3 — closure-cell registry evicted on `graph.remove`/`destroy`/`impl.close` (one mechanism, wired into existing `nodesByName` teardown path); F4 — drop `_dispose` entirely per value #1 (sweep `packages/parity-tests/` for callsites; replace each with `close()` in same slice — value-#1 clean cut).
  - **Acceptance bar:** substrate-internal zero changes (D292 is a binding-layer slice); 5 new cargo regressions in `crates/graphrefly-bindings-js/` (`d292_async_commit_panic_propagates_as_rejection` — REQUIRES TSFN-backed sink fixture, NOT just `Rc<dyn Fn>` since D291's `Rc` test didn't exercise libuv-vs-actor handoff path that causes Case 15a's deadlock; `d292_async_rollback_panic_propagates_as_rejection`; `d292_close_drains_inflight_batch`; `d292_finalize_async_off_libuv`; `d292_auto_close_on_before_exit_opt_in`); `Impl.derived(g, name, deps, fn)` widening on `packages/parity-tests/impls/types.ts`; drop D287 + D291 Case 15a `runIf` gates; new `scenarios/usage/close-drains.test.ts`; `mise run gate` (graphrefly-rs) + `pnpm --filter @graphrefly/parity-tests test` GREEN cross-arm. On close: archive cross-track-ledger §1 D282 / D283 / D293 / (new) D292 rows + `~/src/graphrefly-ts/docs/optimizations.md` D292 LOCKED block → `archive/optimizations/resolved-decisions.jsonl` id `d292-native-async-lifecycle-locked-2026-05-25`.
  - **Rejected alternatives** (full set preserved in SESSION-DS): D.1 = B (regresses `nodeCount=3` parity) / C (perf optimization, defer per D196); D.2 = B (loses D288 amortization) / C (requires substrate "stable BatchGuard id" widening, no precedent); D.3 Item 4 = B (silent data loss) / C (defer per consumer pressure); D.3 Item 5 = A (locks pre-1.0 semantic; unreliable under jest/deno/pool configs) / C (defeats D293's UX win).
  - **Recorded in memory `project_next_porting_batch.md`.**
- **E** — Open `porting-deferred.md` items (mostly v1 limitations the user accepted at design time; each has its own lift-point note). Notable post-D291: `has_received_teardown` rollback + `pause_state` drain rollback + `replay_buffer` drain rollback (D291 deferred-scope; lift on consumer pressure beyond Case 5).
...
**Canonical decision log:** `~/src/graphrefly-ts/docs/rust-port-decisions.md` (D193–D291 in scope for the current Rust track).
...
**Open work (2026-05-25):** see the track-status table above, **Real `/porting-to-rs` options remaining** (A–E), and `docs/porting-deferred.md`. Historical Unit-7 Q9.2 items (F24, D171, stratify) are closed — do not re-queue from this section.

### HISTORICAL — Ship `@graphrefly/native` (D203 native-publish milestone; CLOSED 2026-05-21)

> **Archive only.** Steps 1–8 below completed (`@graphrefly/native` published — npm latest `0.0.3`; bindings crate source at `0.1.0` per D265 hold-local awaiting next user-gated republish — plus Option C wrapper + parity arm green). Kept for audit trail; do not treat as pending work.
...
### D291 — Bucket A `/porting-to-rs` slice: D290 follow-ons (Cases 5 + 15a) + Arc→Rc tail verify + CI tripwire-wiring guard (2026-05-25, PARTIAL closure of cross-track-ledger §1 D282 — Case 5 cross-arm; Case 15a stays open pending D292)

Closes Case 5 cross-arm in `batch-throw-rollback.test.ts`; lands the Case 15a substrate Q2 tripwire-ordering fix BUT cross-arm vitest verification surfaced a separate libuv-sync-commit deadlock (sync `BenchBatchContext::commit` blocks libuv → TSFN-backed JS sinks can't fire during `BatchGuard::Drop`'s success path), so Case 15a stays `runIf(impl.name === "pure-ts")` pending D292's async-commit conversion. Also verifies D272/D273's Arc→Rc sweep is complete (no further work), and adds a CI-hygiene tripwire-wiring grep guard for D289 QA F2.

- **Item 1 — Case 5 (substrate, `graphrefly-core`).** Closes R4.3.2 status-snapshot completeness for terminal slots. New `wave_terminal_snapshots: AHashSet<NodeId>` + `wave_dep_terminal_snapshots: AHashSet<(NodeId, usize)>` on `WaveState` track the `None → Some(_)` transitions in [`Core::terminate_node`](../crates/graphrefly-core/src/node.rs) (entry + cascade to children's per-dep slots); snapshot inserts are gated on `in_tick` (mirrors `commit_emission`'s cache-snapshot gate). New `Core::restore_wave_terminal_snapshots(s, ws) -> Vec<HandleId>` resets the slots to `None` and returns any `TerminalKind::Error(h)` handles for lock-released release; wired into [`BatchGuard::discard_wave_cleanup`](../crates/graphrefly-core/src/batch.rs) alongside the existing cache restore. Success-path `BatchGuard::drop` calls new `Core::drain_wave_terminal_snapshots(ws)` (clears snapshot sets without touching slots — commit keeps the wave's mutations). `Core::in_tick` promoted from private to `pub(crate)` so `terminate_node` can call it. Outermost-wave-start debug_asserts added for both new fields. 3 cargo regressions in [`crates/graphrefly-core/tests/d291_terminal_rollback.rs`](../crates/graphrefly-core/tests/d291_terminal_rollback.rs): leaf-source COMPLETE rollback (Case 5 minimum); state→derived cascade rollback (covers per-dep slot); success-path commit sanity. **DEFERRED scope (D291 does NOT cover; logged in `porting-deferred.md` §"D291 deferred-scope"):** `rec.has_received_teardown` flag, `pause_state` buffer drain, `replay_buffer` drain — Case 5 doesn't exercise; lift on real consumer pressure.

- **Item 2 — Case 15a substrate fix LANDED; cross-arm lift PARTIAL (binding, `graphrefly-bindings-js`).** Closes the commit-path Q2 tripwire ordering gap. [`run_batch_loop`](../crates/graphrefly-bindings-js/src/batch_bindings.rs) renamed `_handle_guard` → `handle_guard` and the `BatchOp::Commit` arm now does explicit `drop(handle_guard)` BEFORE `drop(guard_holder.take())`, so `DURING_BATCH_HANDLE` is clear when `BatchGuard::Drop`'s success path runs `fire_deferred` (R4.3.5 coalescing). Rollback path's decl-order reverse-drop preserved (Q2's load-bearing invariant on `discard_wave_cleanup` intact — `resume_unwind` never reaches the explicit-drop site; `handle_guard` declared FIRST → drops LAST under unwind). Doc-comment split into commit-path vs rollback-path rationale. New cargo regression `d291_commit_path_clears_during_batch_handle_before_sinks_fire` installs a Rust `Rc` sink that snapshots `DURING_BATCH_HANDLE` at fire time; post-fix all snapshots read `false` (was `true` pre-fix). A companion negative test for the rollback-path invariant was attempted then dropped — `resume_unwind` doesn't invoke the panic hook, the substrate panic-discard doesn't fire sinks, and panic-discarded `OnInvalidate` hooks don't fire either; the invariant is structurally pinned via the doc-comment + existing `assert_no_batch_handle_panics_when_flag_set` mechanism test. **Cross-arm lift blocker (NEW, surfaced post-fix during vitest verification):** `BenchBatchContext::commit` is sync napi — `rx.recv()` blocks libuv → `BatchGuard::Drop`'s success-path `fire_deferred` cannot reach TSFN-backed JS sinks → 3-way deadlock between JS commit caller, libuv, and the parked actor. The D289 substrate test passes because it uses Rust-side `Rc` sinks; only real TSFN-backed JS consumers expose the deadlock. Case 15a stays `runIf(impl.name === "pure-ts")` in `scenarios/core/batch-throw-rollback.test.ts:928`. Lift requires converting `BenchBatchContext::commit` + `rollback` to async napi (D292 follow-on slice — mirrors D267's `BenchGraph` async-read fix). See [`docs/porting-deferred.md`](porting-deferred.md) § "D291 → D292 carry" + graphrefly-ts `docs/optimizations.md` "🔴 DESIGN SESSION FIRST" item.

- **Item 3 — Arc→Rc cleanup tail (verify-and-document, no code change).** Empirical scan via `grep -rn arc_with_non_send_sync` shows only 3 `#[allow(clippy::arc_with_non_send_sync)]` annotations remain across the workspace, ALL with rationale comments naming the D273 Cat-1/2 classification:
  - `crates/graphrefly-operators/src/temporal.rs:1401` — `Arc<Mutex<...>>` for `current_inner` captured by `em.defer(move |c| ...)`'s `Send+'static` bound (tokio-driven `WindowTimeCmd` loop).
  - `crates/graphrefly-operators/tests/common/mod.rs:830` — `Arc<RecorderInner>` for the `Weak`-upgrade pattern in the sink registry.
  - `crates/graphrefly-core/tests/lock_discipline.rs:198` — `Arc<TestRuntime>` for test sink-closure capture across closure boundaries.
  Each is a known-correct retain; the original "remaining sites in graphrefly-graph/describe + operators/*" framing in the prior Track-status snapshot was stale pre-D273 doc text. No further substrate work in this slice; Track-status snapshot updated to mark this option resolved.

- **Item 4 — CI tripwire-wiring grep guard (D289 QA F2 closure).** New [`scripts/check-tripwire-wiring.sh`](../scripts/check-tripwire-wiring.sh) walks every `fn bridge_sync*` declaration in `crates/graphrefly-bindings-js/src/*.rs` (awk-based, brace-counting with a `body_started` flag that correctly handles multi-line `where` clauses BEFORE the opening `{`), and fails-loud if any body lacks an `assert_no_batch_handle(` call. New `[tasks."check:tripwire-wiring"]` in `.mise.toml`. Wired into `gate.sh` as Step 0 (fast pre-flight, fails before any cargo compile). Pinned by a throwaway fixture during development (good-fn passes, bad-fn fails with `VIOLATION` + correct file:line:fn-name). Closes the D289 QA F2 follow-on.

- **Bucket B (recorded as NEXT BATCH FIRST ITEM, NOT in this slice):** D — `BenchGraph` arbitrary-fn `g.derived(name, deps, fn)` TSFN-dispatch widening (lifts D287 carve-out on `resource-profile.test.ts` test #1 cross-arm). Requires its own design session BEFORE implementation per Q1–Q3 sub-decisions (closure-cell lifetime, JS-side wrapper integration shape, `Impl` widening surface). See "Real options remaining" table above + memory `project_next_porting_batch.md`.

**`Impl` / napi widening:** none. The new `wave_terminal_snapshots` + `wave_dep_terminal_snapshots` are `pub(crate)` fields on substrate-internal `WaveState`; restore functions are `pub(crate)` on `Core`. `BenchBatchContext`'s public napi surface unchanged. Cross-arm `Impl.batch(fn: (ctx) => void)` shape unchanged.

**Cross-track-ledger:** §1 D282 row PARTIAL closure — status updated to "TS ✅ + native ✅ Case 5 aligned 2026-05-25 (D290 + D291 Item 1); **Case 15a stays open pending D292 async-commit slice**." Row stays open; do NOT archive yet (close after D292 lands).

**Tests:** +3 substrate (`d291_terminal_rollback.rs`) + +1 binding (`d291_commit_path_clears_during_batch_handle_before_sinks_fire`). Gate full GREEN: 869/869 default-members nextest --profile ci (was 866 + 3); 6/6 bindings-js (was 5 + 1, minus the dropped negative test). `cargo clippy -p graphrefly-core -p graphrefly-bindings-js --all-targets` clean. `cargo fmt --check` clean. `#![forbid(unsafe_code)]` preserved. Tripwire-wiring guard clean.

**D265 hold-local:** graphrefly-rs commits for D291 are LOCAL ONLY; not pushed. User-gated tag push triggers OIDC `@graphrefly/native` republish (would re-include the D289 + D290 wrapper.js + this slice's binding fix + substrate fix).

**Canonical:** `~/src/graphrefly-ts/docs/rust-port-decisions.md` D291 + D290 + D289 + D288 + D282 + R4.3.2; cross-track-ledger §1 D282 row (PARTIAL closure, Case 15a open); test pins per above. Case 15a follow-on canonical in `docs/porting-deferred.md` § "D291 → D292 carry — `BatchOp::Commit` panic-during-drop hang".
...
- **§10.3 (diamond resolution bookkeeping) → RESOLVED (validated not a win).** New `crates/graphrefly-core/benches/diamond_resolution.rs` paralleling `notification_buffer.rs`. Sweep fan-in N = 1, 4, 8, 16, 32, 64 over the classic `dispatcher.rs::build_diamond` topology (1 source → N intermediates → 1 sink). Numbers: 910 / 1796 / 2983 / 5586 / 11364 / 26617 ns per emit. Per-dep marginal cost stabilizes ~350 ns from N=8 onward; slight super-linear knee at N=64 (2.34× time for 2× deps) attributable to cache effects, not AHashSet load-factor. The proposed `WaveTracker { settled: u64, all_deps_mask: u64 }` bitmask would replace AHashSet insert/contains with bit-ops, theoretical max savings ~30–50 ns per dep — but per-dep cost is dominated by wave-engine fire overhead (commit_emission + queue_notify + deliver_data_to_consumer + sink flush + dispatch), not AHashSet. Clean below the ≥5% bar at every sample point. Closed.
- **§10.6 (sink iteration) → RESOLVED (validated not a win).** New `crates/graphrefly-core/benches/sink_iteration.rs`. Sweep subscriber count S = 1, 8, 32, 128, 512 on a single state node. Numbers: 483 / 633 / 789 / 1532 / 4071 ns per emit. Per-sink marginal cost stabilizes at ~7 ns from S=32 onward (S=128→512 is 2.66× time for 4× sinks — sub-linear); the cost is consistent with `Rc<dyn Fn>` closure-dispatch overhead and shows no super-linear HashMap pressure, though the bench measures the SUM (per-wave snapshot build + SmallVec walk + closure call) and cannot decompose into constituents — further attribution would need a component bench. The original "flush iterates under lock" framing is stale: post-D108 Q-beyond, the sink-call loop already runs lock-released via `deferred_flush_jobs`; only the per-wave snapshot BUILD at `batch.rs:3038-3043` touches `subscribers` under lock. The proposed epoch/snapshot-spread refactor would eliminate that build, but the bench shows it is already amortized across all batches in the wave with no super-linear scaling; to clear the ≥5% bar at S=512 it would need to save ~204 ns — well inside noise. Late-subscriber correctness stays handled by D2/Slice-X4's per-batch sink snapshot. **Methodology:** S=1 sits on the `SmallVec<[Sink; 1]>` inline branch; S≥8 spills — per-sink slope read from S∈{8,32,128,512}. `release_handle` no-op in bench binding — substrate-internal cost only (see §10 table footer). Clean below the ≥5% bar at every applicable sample point. Closed.
- **D252 (b) — panic-msg shape amend (option (a)).** `crates/graphrefly-core/src/batch.rs` `claim_in_tick` panic message reduced from 4 paragraphs (general invariant + Rust callers + @graphrefly/native + internal reference) to 3 paragraphs (general invariant + Rust callers with parenthetical BenchCore hint + internal reference). The multi-paragraph JS-consumer framing was cosmetic surface for an unreachable audience: `core_actor.rs` M1's `catch_unwind` swallows the panic value before it crosses napi (per D262/P4 framing). The comment block above the panic was reframed to lead with "Audience: Rust-dev clarity only" instead of layered amendment history. **No behavioral change** — panic still fires under the same trigger and still carries the same diagnostic load on the Rust panic hook → stderr. D252 entry in `porting-deferred.md` flipped from "partially resolved by S6" to fully strikethrough'd RESOLVED 2026-05-22.

**`Impl` / parity-tests / napi widening:** none. Two benches added (numbers only; no production code change) + one panic-message text edit + 2 doc files updated.

**Cross-track-ledger row:** none. Substrate-internal.

**Gate:** `mise run gate` GREEN (full nextest `--profile ci` + clippy + fmt) — **850 tests passed, 0 skipped**, exit 0 (428s). `cargo clippy --all-targets -- -D warnings` clean. `cargo fmt --check` clean. `#![forbid(unsafe_code)]` preserved. The 832 → 850 baseline delta is post-D271 D275/D276 test additions (storage-honest-error + M4.E1 cross-mount snapshot), not from this slice (benches don't add tests).
...
- **B — `AppendLogStorage::flush` rollback-epoch re-check in error-restore paths.** Added a `restore_or_drop` helper closure local to `flush()` that re-checks `rollback_epoch` before re-inserting the bucket back into `pending` on any of the 4 error-restore paths (`backend.read` / `codec.decode` / `codec.encode` / `backend.write`). If the epoch advanced during the failing operation, the bucket is dropped — the user's `rollback()` semantic wins (consistent with the existing per-bucket-loop epoch abort at flush top). 1 cargo test using a barrier-synced `FaultBarrierBackend` (`crates/graphrefly-storage/tests/tier.rs::append_log_rollback_during_write_error_drops_bucket_not_restores`).

- **C — `AppendLogSink::mode()` + `attach_storage` rejects Overwrite.** Widened `AppendLogSink<T>` with `fn mode(&self) -> AppendLogMode { Append }` (default-impl back-compat). `ReactiveLog::attach_storage` signature widened to return `Result<AttachStorageHandle<T>, AttachStorageError>`; rejects any `Overwrite` sink at entry with `AttachStorageError::OverwriteSinkRejected { sink_index }`. Local mirror `AppendLogMode { Append, Overwrite }` enum in `graphrefly_structures::reactive` — Q1 user-locked local mirror because `graphrefly-storage` already depends on `graphrefly-structures` for `BaseChange<T>`, blocking forward from storage (cycle). 2 cargo tests (`log_attach_storage_rejects_overwrite_sink`, `log_attach_storage_rejects_first_overwrite_in_mixed_sinks`).

- **D — `AppendLogSink::flush()` + `AttachStorageHandle::flush_all()` (F9 parity, Option B per D171).** Widened `AppendLogSink<T>` with `fn flush(&self) -> Result<…> { Ok(()) }` (default no-op). `AttachStorageHandle` is now generic over `T` and retains shared sinks; gained `flush_all() -> Result<…>` driving each sink's `flush()` sequentially, surfacing the first error with a `sink[N] flush:` prefix and short-circuiting on failure (matches `graphrefly_storage::StorageHandle::flush_all`). **No tokio added** — callers wire `interval(debounce_ms) → handle.flush_all()` from a reactive timer subgraph (Option B per D171 — explicitly NOT a built-in driver inside the substrate). 2 cargo tests (`log_attach_storage_flush_all_drives_each_sink_flush`, `log_attach_storage_flush_all_surfaces_first_error_with_index`).

**`Impl`/parity-tests/napi widening:** none. F18 (`attach_storage` napi) stays deferred per D196 (no consumer scenario). Substrate-only slice; no cross-track-ledger row added.

**Pre-existing impl touched:** the only `AppendLogSink` impl in-tree is `TestSink` in `crates/graphrefly-structures/tests/reactive_structures.rs`; only the `attach_storage(...)` call site needed an `.expect(…)` to consume the new `Result` (no other downstream impact).

**Gate:** `mise run gate` (full nextest `--profile ci` + clippy + fmt). `cargo clippy -p graphrefly-storage -p graphrefly-structures --all-targets` clean. `cargo fmt --check` clean. `#![forbid(unsafe_code)]` preserved.

**Canonical:** `~/src/graphrefly-ts/docs/rust-port-decisions.md` D271.

### Cross-track-ledger closure — D266-D270 cross-arm parity follow-on (2026-05-21)

The cross-arm parity scenarios deferred during the substrate-side D266-D270 closure (see the next block, "all 5 open ledger rows" batch) were authored as a follow-on `/porting-to-rs` + `/qa` pass on 2026-05-21:

- **`packages/parity-tests/scenarios/storage-wal/durability-mode-pagination.test.ts`** — 14 scenarios × 2 arms (28 tests). Covers D268 (rollback-clears-pending, rollback-post-flush no-op, epoch-advanced re-flush) + D269 (mode accessor for default/'append'/'overwrite'; overwrite-replaces-bucket-per-flush; `loadEntriesPaged` cursor-walk; pageSize larger than log; cursor past end; empty log; bare `loadEntries()` back-compat).
- **`packages/parity-tests/scenarios/messaging/skip-cached-replay.test.ts`** — 5 scenarios × 2 arms (10 tests). Covers D270 default attach (cached replay), `skipCachedReplay:true` on cached upstream (handshake suppressed; subsequent live emits still land), cold-upstream no-op (first live emit DOES land — gated on cache-present), and explicit `skipCachedReplay:false`.

Parity gate result: **38 files / 415 tests / 1 intentional skip** green cross-arm. The rust-via-napi arm exercises the new shapes via `wrapper.js`'s `loadEntriesPaged` napi path and `attach(upstream, skipCachedReplay)` napi path. The deferred `porting-deferred.md` entry "Cross-arm parity scenarios for D268/D269/D270" is now resolved.

### Cross-track-ledger closure — D266/D267/D268/D269/D270 (2026-05-21, `/porting-to-rs` "all 5 open ledger rows" batch)

**All 5 open native-pending rows in `~/src/graphrefly-ts/docs/cross-track-ledger.md` resolved in a single back-to-back `/porting-to-rs` pass (D266 staging lock).** Local commits pending in graphrefly-rs; not pushed (publish-gate per D265 pattern; user-gated tag push triggers OIDC publish).

1. **🟠 `Graph::remove`/`destroy` napi TSFN deadlock — D267, F1 fix.** Root cause CONFIRMED via reproduction (not D070/D077 directly — sink-callback re-entrance into `actor.run_sync`). Every sync `#[napi]` method on `BenchGraph` (`name`/`node_count`/`node_names`/`child_names`/`is_destroyed`/`try_resolve`/`name_of`/`edges`/`describe_json`) + `BenchCore::describe_node` promoted from `actor.run_sync` to `async fn`+`actor.run`. Parity `Impl` widened to `T | Promise<T>` for `tryResolve`/`nameOf`/`edges`/`describe`/`describeNode` so the pure-ts arm returns sync unchanged. JS-side reverse cache `NativeGraph.nodeIdToName` populated on `state`/`add`, cleared on `remove`/`destroy` AFTER `bench.remove/destroy` resolves — preserves R3.7.3 sync-observability of names from inside the TEARDOWN sink cross-arm. Constructor `from_core` + structures `create` factories keep `run_sync` (one-shot, no subscribers — documented safe). Parity suite **36/36 files, 377 tests + 1 skip green cross-arm** (previously-excluded `remove.test.ts` now runs cross-arm). Cross-ref: `docs/rust-port-decisions.md` D267.

2. **memo:Re P0 storage durability + reject + F9 driver + rollback epoch — D268.** (a) Durability-on-resolve: structurally satisfied — Rust `AppendLogStorage::flush` is sync, no async chained-microtask hazard. (b) Error-surfacing: structurally satisfied — `flush`/`append_entries` return `Result<(), StorageError>`. **/qa-fix 2026-05-21:** the error-restore path in `flush()` previously restored `final_payload` (existing-read-from-backend ⊕ new entries) to `pending` on encode/write failure; on retry, flush re-read existing AGAIN and merged → existing entries silently duplicated. Fixed: split into `(final_payload, restore_payload)` — Append-mode restores `bucket_backup` (new entries only); Overwrite-mode restores the bucket snapshot. Regression test `append_log_append_mode_write_failure_does_not_duplicate_on_retry` uses a `FaultBackend` to inject a write fault, then asserts the retry's persisted bucket is `[1,2,3,4]` not `[1,2,1,2,3,4]`. (c) F9 reactive-timer-driven debounce-flush driver: **no built-in driver shipped in this slice.** The pattern is documented for callers (drive `flush()` from `graphrefly_operators::temporal::interval`, mirroring the existing `attach_snapshot_storage` precedent); built-in driver inside `attach_storage` is deferred until the F18 `attach_storage` napi slice — recorded in `porting-deferred.md`. (d) Rollback epoch: `AppendLogStorage::rollback_epoch: AtomicU64`; `rollback()` bumps epoch + clears pending; `flush()` captures epoch at start, checks before each per-bucket write, aborts on bump — closes the concurrent rollback-during-flush window. 2 pre-/qa cargo tests (sequential rollback-then-flush; deterministic concurrent rollback-during-flush via BarrierBackend) + 1 /qa-regression test (write-fault retry). Cross-ref: `docs/rust-port-decisions.md` D268.

3. **memo:Re P1 mode + `attach_storage(overwrite)` rejection — D269 (mode part).** New `AppendLogMode { Append, Overwrite }` enum in `graphrefly_storage::tier`; `AppendLogStorageOptions.mode` field (default `Append`); `AppendLogStorage::flush()` `Overwrite` branch skips read-merge — bucket IS the full contents. `AppendLogStorageTier::mode()` accessor exposes the configured mode. Napi: `BenchValueAppendLogTier::create` accepts `mode_str: Option<String>` ("append"/"overwrite"); `BenchValueAppendLogTier::mode` (`#[napi(getter)]`). Wrapper widened. Parity `TierOpts.mode` + `ImplAppendLogTier.mode` widened. 2 cargo tests. **`attach_storage` rejection of overwrite tiers — DEFERRED:** structurally the substrate `ReactiveLog::attach_storage` doesn't enforce this; documented in `porting-deferred.md` as a follow-on (the binding-layer attach_storage napi is also deferred per F18 — no consumer scenario yet — so the rejection point shifts when that lands). Cross-ref: `docs/rust-port-decisions.md` D269.

4. **`appendLogStorage.loadEntries` windowed cursor pagination — D269 (pagination part).** New types: `AppendCursor { position: u64, tag: Option<u64> }`, `LoadEntriesOpts<'a> { cursor, page_size, key_filter }`, `AppendLoadResult<T> { entries, cursor }`. Trait method `AppendLogStorageTier::load_entries(opts)` returns `AppendLoadResult<T>`. Pre-D269 `load_entries(key_filter)` shape preserved as `load_entries_all(key_filter)` (call sites swept). Deterministic lex-ASC backend key sort + by-key flatten + `[start, start+pageSize)` window + forward-only cursor. Decoding short-circuits past `start + page_size + 1` to bound consumer working set. Napi: `BenchValueAppendLogTier::load_entries(key_filter, cursor_position, page_size)` — paginated returns `{ "entries": [...], "cursor": ... | null }`; bare returns the entries array (back-compat). Wrapper exposes both `loadEntries(keyFilter)` (back-compat) and `loadEntriesPaged({ keyFilter, cursor, pageSize })`. 4 cargo tests (windowed walk; bare back-compat; cursor-past-end; pageSize>log). Parity `ImplAppendLogTier.loadEntriesPaged` widened. Cross-ref: `docs/rust-port-decisions.md` D269.

5. **memo:Re P2 `ReactiveLogBundle.attach skipCachedReplay` — D270.** New `graphrefly_structures::AttachOptions { skip_cached_replay: bool }`. New `ReactiveLog::attach_with_options(core, upstream, read_value, opts)`; existing `attach()` delegates with default opts (back-compat). Suppression gated on `Core::cache_of(upstream) != NO_HANDLE` at attach time; per-batch sink checks-and-clears `parking_lot::Mutex<bool>` skip flag on the first DATA-bearing batch. Napi: `BenchValueLogBundle::attach(upstream, skip_cached_replay)`. Wrapper widened. Parity `ImplReactiveLog.attach(upstream, opts?)` widened. 3 cargo tests: cached value drops handshake; cold upstream still emits first live DATA; default attach replays. Cross-arm sub-cases not covered (`replayBuffer:N` — Rust substrate has no `replayBuffer` shape; `attach` inside `batch()` — needs additional substrate support for an in-wave `attach`) — documented in `porting-deferred.md`. Cross-ref: `docs/rust-port-decisions.md` D270.

**Decisions:** D266 (staging lock — 4 stages back-to-back), D267 (Stage 1 F1 fix shape), D268 (rollback epoch), D269 (mode + pagination), D270 (skipCachedReplay). Canonical: `~/src/graphrefly-ts/docs/rust-port-decisions.md`.
...
- **B-2 Step 2b-i — LANDED + GREEN (2026-05-16)** = graphrefly-rs
  `84986d8`. `LockedCell` now = `Mutex<CoreShared>` + an explicit
  `default_shard: Arc<parking_lot::Mutex<CoreState>>` (the
  `ShardKey::None` fast path — single `lock_arc`, **zero map/hash/
  or_insert**, D220-EXEC zero-regression) + `grouped_shards:
  Mutex<HashMap<SerializationGroupId, Arc<Mutex<CoreState>>>>`
  (`Some` only; map held only to clone the shard `Arc`, dropped
  before locking it — never serializes waves) + a `binding` clone.
  `ShardGuard` is the OWNED `'static` `parking_lot::ArcMutexGuard`
  via `lock_arc` — **the D220-EXEC "Guard-lifetime crux" is solved**
  (arc_lock feature, zero `unsafe`, GAT `'a` unused).
  `SingleThreadCell` unchanged (single `RefCell`) ⇒ the §7 floor is
  preserved **by construction**. `CoreState::empty_shard(binding)`
  ctor added. **Step 2b-i routes `None` only (via `lock_state`/
  `St`) ⇒ behaviour-identical** (one populated shard;
  `grouped_shards` stays empty). Gates (undisturbed): `mise run
  gate` rustfmt ✓ / clippy ✓ clean / `nextest --profile ci`
  **825/825, 0 skipped** (80 s); `cargo bench floor_compare`
...
> QA (`docs/porting-deferred.md` § "S7 follow-on"). The bug class —
> mailbox post stranded past a sink-fire boundary — manifested in two
> code paths plugged by D260 + D261. Final state: `mise run gate`
> **812/0/0 GREEN** (was 810; +2 new D260/D261 regression tests),
> `pnpm test:parity` rust-via-napi arm **34/35 files green** (the 1
> remaining is a pre-existing pure-ts `switchMap` bug — rust passes
> the same test cross-arm, carried to next `/dev-dispatch` per
> memory `project_next_optimizations_batch.md` item ③).

**Root cause (both decisions):** Post-S2b/S2c, producer-build sinks
(`graphrefly_operators::buffer::buffer`'s `notifier_sink`, the
reactive-log `view`'s internal sink, etc.) capture `ProducerEmitter` /
`SinkEmitter` and emit via `mailbox.post_emit(...)` instead of the
pre-S2b `core.emit_or_defer(...)` direct-call. The mailbox is meant to
be drained by the wave's `drain_and_flush` loop, which iterates while
`mailbox.is_runnable() || deferred.is_runnable()`. But:

1. (D260) Sinks queued into `WaveState::deferred_flush_jobs` fire in
   `fire_deferred`, which runs **after** `drain_and_flush` exits. A
   producer-build sink firing in `fire_deferred` and posting via
   mailbox left the post stranded until the **next external wave
   entry**. Subscriber sinks on the producer's output node never fired
   within the same outer wave.
2. (D261) `Core::try_subscribe`'s per-tier handshake-fire phase
   (R1.3.5.a `[Start]/[Data(cache)]?/[Complete]?/[Error(h)]?/[Teardown]?`
   slices) fires sinks lock-released **outside any `BatchGuard`**. A
   sink that posted via mailbox during push-on-subscribe replay (e.g.
   the reactive-log `view`'s internal sink replaying the log's cached
   state to compute its initial window) left the post stranded with
   no `BatchGuard::Drop` to trigger D260's loop.

**What landed:**

- **D260** ([`crates/graphrefly-core/src/batch.rs`](../crates/graphrefly-core/src/batch.rs)
  `BatchGuard::Drop`): wraps the existing `drain_and_flush → extract
  → clear_in_tick → fire_deferred → release` sequence in a `loop`
  that re-claims `in_tick` and repeats while `mailbox.is_runnable()
  || deferred.is_runnable()` post-`fire_deferred`. Bounded-iteration
  via the existing `max_batch_drain_iterations` cap per pass; same
  hazard profile as pre-S2b's nested-wave recursion (user emit-loop
  is a stack overflow there, an iteration-bounded loop here).
- **D261** ([`crates/graphrefly-core/src/node.rs`](../crates/graphrefly-core/src/node.rs)
  `Core::try_subscribe`): after the per-tier handshake-fire loop,
  before activation, enters a brief `BatchGuard` gated on
  `mailbox.is_runnable() || deferred.is_runnable()`. The guard's Drop
  runs the D260 loop. Cost on the no-post path: one `runnable` atomic
  load per subscribe.
- **D258 fold-in** ([`batch.rs:3490`](../crates/graphrefly-core/src/batch.rs)
  `claim_in_tick` panic): D252 hard-invariant panic message reworded
  for clarity. **/qa correction (D262, 2026-05-20):** the original S7
  framing "useful for both in-tree Rust devs and `@graphrefly/native`
  consumers" is **overstated**. `core_actor.rs:354-357`'s M1
  `catch_unwind(...)` discipline runs `let _ = std::panic::catch_unwind(
  AssertUnwindSafe(|| closure(&core)))` — the panic value is **swallowed
  by `let _ =`**; the polished message reaches Rust's default panic hook
  (stderr `eprintln!`) but never crosses the napi boundary. JS callers
  observe a sync_channel disconnect / actor reply error, not the
  message. D258's actual value-add is therefore **Rust-dev clarity
  only** (someone debugging a binding-layer misuse from the cdylib's
  stderr); the binding-user-facing framing is **aspirational pending
  a future panic→JS bridge slice** (not committed). Gap (a) (pre-claim
  handle-leak) was structurally resolved by D255's per-`BenchCore`
  actor thread in S6 — that part of D258 stands.
- **Regression test** ([`crates/graphrefly-core/tests/d260_d261_wave_end_mailbox_drain.rs`](../crates/graphrefly-core/tests/d260_d261_wave_end_mailbox_drain.rs)):
  2 tests pinning the D260 fire_deferred re-drain + D261 handshake-fire
  drain invariants. Mirrors the failure shapes of `buffer::notifier_sink`
  (D260) and `reactive_log::view::internal_sink` (D261).
- **Test update** ([`lock_released.rs`](../crates/graphrefly-core/tests/lock_released.rs)
  `late_subscriber_installed_after_first_queue_notify_does_not_double_receive_data`):
  the test's pre-D260 expected list `[1, 2]` was incidentally tied to
  the mailbox-stranded artifact (Defer-subscribe deferred to next
  external wave, where `s.cache` was already advanced). Post-D260
  in-wave Defer-application restores pre-S2b's nested-wave timing →
  late sees `[0, 1, 2]`. The load-bearing X4/D2 freeze invariant (no
  DUPLICATE values from frozen `PendingBatch` flush) is preserved
  with the new expected `[0, 1, 2]` + a defensive `.dedup() == .len()`
  assertion.

**Diagnosis path (recorded for future debug discipline):** Phase A
instrumentation (`GRAPHREFLY_S7_TRACE=1` env-gated `eprintln!`s at
`bridge_sync` / `bridge_sync_unit` / `build_tsfn_sink` /
`core_actor.rs` worker loop / `fire_deferred` entry) ruled out the
4 originally-listed hypotheses in `porting-deferred.md` (H1 M1
`catch_unwind` swallowing, H2 TSFN queue saturation under nested
actor + libuv, H3 sink Arc lifetime, H4 `BatchGuard` panic-discard
catching actor caller's `Ok(())`). All four false. Traces surfaced
H5 — uniform "emissions lost" pattern with bridge_sync packer running
cleanly + fire_deferred completing cleanly + NO subsequent sink-FIRE
on the producer's output node — pinning the bug at "mailbox post in
fire_deferred stranded past wave end."

**Final gate state:**
- `mise run gate` → ✅ **812 passed / 0 skipped / 0 failed** in 16s
  (full elapsed; `--profile ci`; sentinel exit=0/reason=ok at
  `.gate/gate-full-20260520-150502.log`).
- `pnpm test:parity` (rust-via-napi arm) → 34/35 scenario files
  green; 1 file 17/18 (pure-ts switchMap, cross-arm-NOT-rust failure,
  spawned to /dev-dispatch).
- `cargo clippy -p graphrefly-core --all-targets` → 0 errors; 1
  pre-existing warning in `tests/lock_discipline.rs` (Arc-of-non-Send
  closure, unrelated to S7 changes).
- `cargo fmt --check` → clean.
- `pnpm build` (napi cdylib) → ✅ green.
- `#![forbid(unsafe_code)]` preserved in graphrefly-core;
  `#![deny(unsafe_code)]` preserved in graphrefly-bindings-js.

**Process learning surfaced & preserved:** the user-feedback memory
`feedback_pre_design_full_decision_set.md` rule #5 (binding-layer
slices need `pnpm test:parity` as actual coverage gate, not just
substrate `mise run gate`) was load-bearing this slice — the 35
failures were invisible to substrate gate and only surfaced via
parity runtime. Continued in S7's debug-first protocol: Phase A
instrumentation traces + parity gate as the load-bearing surface,
substrate gate as the secondary sanity check.

**Decisions:** `~/src/graphrefly-ts/docs/rust-port-decisions.md`
D259 (S7 debug-slice shape — Phase A instrumentation first, Q1=A,
Q2=HALT-on-H4, Q3=D258 fold-in, Q4=light shape with HALT-on-structural-
fix gate), D260 (wave-end re-drain loop in BatchGuard::Drop, Option A),
D261 (handshake-fire mailbox drain in try_subscribe, Option D —
surfaced post-D260 as the same bug class at a different boundary),
**D262 (S7 /qa hardening + canonical lock — outer-loop cap 32 / panic
isolation in fire_deferred / release-build !owns break / Rust-canonical
timing lock; A2 verify-via-existing-parity-scenarios not via cross-track-
ledger row; A3 Rust canonical, TS/PY must converge if they expose
equivalent mailbox seam)**.

**Rust-canonical timing lock (A3, D262 2026-05-20).** Rust IS the
canonical for the wave-end mailbox-quiescence shape — `fire_deferred`-
posted `MailboxOp::Emit` MUST drain within the same outer wave; a
sink-fire in `try_subscribe`'s handshake-replay that posts to mailbox
MUST drain by `try_subscribe`-return. TS/PY MUST satisfy the same
observable invariant if/when they expose an equivalent mailbox-posting
sink seam. Pure-ts today implements the same observable behavior via
direct-call sink dispatch (no mailbox indirection → naturally quiescent
at wave end → no "stranded post" hazard); the existing parity scenarios
(buffer, view(slice), stratify, higher-order, control, subscription,
messaging) verify the observable-shape agreement empirically — 34
scenario files that flipped rust-arm-failing → cross-arm-passing
post-D260+D261 ARE the documentation. Spec R-ID minting (e.g.
R1.3.6.d in `docs/implementation-plan-13.6-canonical-spec.md`) is the
proper long-term home but deferred to the next spec amendment session.

**S7 /qa patches (D262, 2026-05-20).** Adversarial review (Blind
Hunter + Edge Case Hunter parallel subagents) surfaced 3 P0/P1
hardening gaps + 3 doc polish items + 4 verified-no-issue items.
All 6 patches landed:

- **P1** ([batch.rs](../crates/graphrefly-core/src/batch.rs) D260 loop):
  wrap `fire_deferred` in `catch_unwind`. Pre-/qa, a sink panic in
  iteration N escaped the loop bypassing `snapshot_releases`
  retain-release + the wave-end `tier3_clear` / `drain_deferred_producer_ops`
  (D211 no-op shim today but documented contract). Post-/qa, the
  panic is caught, `snapshot_releases` is released defensively
  lock-released, `tier3_clear` + `drain_deferred_producer_ops` run,
  then `resume_unwind` so the caller observes the panic. Mirrors
  the L3844 drain-phase wrap.
- **P2** ([batch.rs](../crates/graphrefly-core/src/batch.rs) D260
  secondary-pass): explicit `if !owns { break; }` after the
  `debug_assert!(owns, …)`. Defense-in-depth — if `claim_in_tick`
  ever silently returned `false` (slot held by this Core's generation
  — should be impossible under D255+D248 by-construction), the
  loop gracefully exits rather than running `drain_and_flush`
  without ownership. The next outer wave entry catches the still-
  runnable mailbox.
- **P3** ([d260_d261_wave_end_mailbox_drain.rs](../crates/graphrefly-core/tests/d260_d261_wave_end_mailbox_drain.rs)
  D261 test): `source_sink_fires >= 1` → `== 1`. Pin the handshake-
  replay single-fire invariant; pre-tighten a future regression that
  double-delivers handshake replay would have silently passed `>= 1`.
- **P4** (this doc + porting-deferred): D258 framing downgrade — see
  above. The polished panic message is Rust-dev-only useful pending
  a future panic→JS bridge slice.
- **P5** ([batch.rs](../crates/graphrefly-core/src/batch.rs) D260
  doc comment): "Why iteration not nested recursion" now leads with
  the same-thread sink case (primary bug surface — the 35 parity
  failures) instead of the cross-thread autonomous-timer-task case
  (secondary motivation for `MailboxEmitter`'s `Send + Sync` shape).
- **P6** ([node.rs](../crates/graphrefly-core/src/node.rs) D261
  inline doc): clarify that `BatchGuard` construction doesn't drain,
  Drop does; add a nested-subscribe note clarifying that a Defer-
  driven `cf.subscribe(...)` correctly gets a non-owning guard with
  no-op Drop (outer wave's drain loop catches the post).

**Outer-loop cap (A1=c, D262 2026-05-20).** D260's outer loop is
now capped at `D260_MAX_REDRAIN_PASSES = 32` (hard-coded constant).
Realistic legitimate wave-end cascades stay ≤3 (each iteration drains
a fresh post produced by the previous `fire_deferred`); 32 gives 10×
headroom and panics loudly with a diagnostic naming the suspect cycle
on exceed. Per-iteration `drain_and_flush` is independently capped
by `max_batch_drain_iterations` (configurable). A user sink emit-loop
was a stack overflow pre-S2b — same hazard, now iteration-shaped
with this cap, fail-loud.

**Post-/qa gate state:**
- `mise run gate` → ✅ **812 passed / 0 skipped / 0 failed** in 185s
  (sentinel exit=0/reason=ok at `.gate/gate-full-20260520-165603.log`).
- `pnpm test:parity` rust-via-napi arm → 34/35 scenario files green
  unchanged (the 1 remaining is the pre-existing pure-ts switchMap
  bug, spawned to /dev-dispatch). All S7 fixes intact post-/qa
  patches.
- `cargo clippy -p graphrefly-core --all-targets` → 0 errors, 1
  pre-existing warning in `tests/lock_discipline.rs` (untouched).
- `cargo fmt --check` → clean.
- `#![forbid(unsafe_code)]` preserved.

**A2 verification record (D262 2026-05-20, user counter-proposal
locked).** The premise of "TS↔Rust observable timing divergence
needing a cross-track-ledger row" was *hypothesized*, not verified.
On re-trace: D260 is the **structural completion** of D232-AMEND's
"drain-to-quiescence in-wave, immediately" contract — Rust under-
delivered pre-D260; Rust is now spec-compliant. Pure-ts has no
mailbox indirection (direct-call sink path → naturally quiescent
at wave end); if pure-ts is spec-compliant, **pure-ts matches
post-D260 Rust** and the lock_released.rs `[1,2] → [0,1,2]` rewrite
is evidence the OLD test expectation encoded the BUG, not evidence
of cross-impl divergence. The 34 cross-arm-passing parity scenarios
(`buffer`, `view(slice)`, `stratify`, `higher-order`, `control`,
`subscription`, `messaging`) ARE the empirical verification. **No
cross-track-ledger row added** (D260 doesn't widen any surface),
**no `cross-language-notes.jsonl` `divergence-*` entry** (no
verified divergence — pre-emptive entries pollute the standing-rule
"drop findings matching existing divergence-* entries"). The user-
visible Defer-driven late-subscribe-during-fire_deferred pattern
isn't directly expressible via the public `Impl` contract (no
`postDefer` seam), so an exhaustive 1:1-shape parity scenario isn't
authored; producer-build operator scenarios (above) cover the same
code path via their internal `MailboxEmitter`/`SinkEmitter` use.

**Carried forward (out of S7 scope, spawned):**

- **Pure-ts `switchMap` completes-when-outer-completes-with-no-active-inner**
  ([`packages/pure-ts/src/extra/operators/higher-order.ts:161`](../../graphrefly-ts/packages/pure-ts/src/extra/operators/higher-order.ts)
  + comment at L71 flags the known gap). Pre-existing pure-ts
  higher-order operator semantics bug, masked by the 35 rust-arm
  failures pre-S7. Rust-via-napi arm PASSES the same test
  cross-impl, so the rust impl IS the spec — pure-ts must catch up.
  Carried to next `/dev-dispatch` batch (memory
  `project_next_optimizations_batch.md` item ③); session spawned.
