...
- **🚢 D293 — patch slice queued AHEAD of D292 (locked 2026-05-25 by D291 /qa option B).** Real-user fix for the "process never exits" hang that hits every napi consumer using a test framework / short-lived script / serverless / Lambda — verified by getting bitten during D291 /qa. Ships as `@graphrefly/native@0.0.8` (patch bump, additive only — no breaking surface). No design session needed: scope is intentionally minimal, decisions pre-locked. D293 LANDS, then D292 design session reshapes/extends without breaking changes.
  - **Substrate:** `MailboxOp::Shutdown` variant + `CoreActor::shutdown(&self)` (sends shutdown op, joins `JoinHandle`, idempotent on already-shutdown). ~50 LOC in `crates/graphrefly-bindings-js/src/core_actor.rs`.
  - **Binding:** `BenchCore::close()` async napi — drains subscriptions (existing `_dispose` logic) → calls `actor.shutdown()` → flips a `closed: AtomicBool` flag. ~30 LOC in `crates/graphrefly-bindings-js/src/core_bindings.rs`.
  - **Post-close method calls:** throw with clear-message ("BenchCore used after close()") — mirrors D288 Q3 "BenchBatchContext used after batch closed" precedent. D292 can later widen to abort-in-flight / drain-vs-cancel without breaking the API.
  - **JS surface:** `wrapper.js` `impl.close()` public + `impl[Symbol.asyncDispose] = impl.close` so Node 22+ users get `await using impl = createNativeImpl()` for free. Existing `_dispose` underscore-prefixed hook stays for the parity harness afterEach (delegates to close internally).
  - **Test:** `packages/parity-tests/scenarios/usage/cleanup.test.ts` — pin (a) standalone-node script exits within 100ms of `await impl.close()`, (b) vitest worker exits without `--forceExit`, (c) double-close is idempotent, (d) method-call after close throws with the documented message.
  - **README:** new "Closing a `NativeImpl`" section showing both the modern (`await using`) and compat (`try { ... } finally { await impl.close(); }`) patterns. Cite the napi-rs non-daemon-thread reality + the alternative `process.exit()` escape hatch.
  - **What stays for D292:** `_dispose` rename to public-only `close` (breaking — minor bump); `FinalizationRegistry` GC fallback; `process.on('beforeExit')` safety net default-on/opt-in lock; richer post-close semantics (abort-in-flight commit Promise / drain-vs-cancel); D.1 `BenchGraph.derived` arbitrary-fn widening; D.2 async `BenchBatchContext::commit`/`rollback` rebase (closes Case 15a libuv deadlock); D.3 `Symbol.asyncDispose` propagation to `Graph` / `Subscription` / `BenchBatchContext`.
  - **Pre-locked decisions (don't re-design in D293):** post-close = throw (mirrors D288 Q3); `_dispose` stays in v0.0.8 as the parity-only hook; `Symbol.asyncDispose` only on `NativeImpl` (not nested handles — D292 territory); no `process.on('beforeExit')` in v0.0.8 (D292 decides default-on/opt-in).
  - **D293 cross-ref:** `docs/optimizations.md` "🚢 D293" entry; memory `project_next_porting_batch.md` D293 block.
