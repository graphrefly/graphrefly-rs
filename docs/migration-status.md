...
8. **N1 — 6 substrate-infra symbols pinned onto `Impl` (cleave /qa, TS side landed 2026-05-15).** The cleave-A dedup fix (`4dd5758`) + A3 barrel additions surfaced 6 symbols on `@graphrefly/pure-ts`'s public **core**/**extra** barrels that the presentation package (`@graphrefly/graphrefly`) imports directly from the substrate peer: **`RingBuffer`, `ResettableTimer`, `describeNode`, `sha256Hex`** (core) + **`sourceOpts`, `wrapSubscribeHook`** (extra). They were absent from the `Impl` parity contract (grep-count 0), so `@graphrefly/graphrefly` breaks the moment `@graphrefly/native` is the substrate provider (native only promises `Impl`). **TS side DONE (cleave session, 2026-05-15):** added to `packages/parity-tests/impls/types.ts` (`ImplRingBuffer`/`ImplResettableTimer` interfaces + 6 `Impl` members) + the `pure-ts.ts` reference arm; `rust.ts` cast widened `as Impl` → `as unknown as Impl` (rust arm structurally incomplete by design until this item). pure-ts arm + lint + 331-test parity gate green. **Native/rust scope (this item):** expose the 6 on the `@graphrefly/native` napi surface (or thin TS wrapper for the non-Rust binding-layer ones — `sourceOpts`/`wrapSubscribeHook`/`describeNode` may be TS-side over the napi core; `sha256Hex` is async-everywhere; `RingBuffer`/`ResettableTimer` are infra classes), populate them in `rust.ts`, then tighten its cast back to `as Impl`. Source: `~/src/graphrefly-ts/archive/docs/SESSION-DS-cleave-A-file-moves.md` § "OPEN /qa FOLLOW-UPS" → N1 (user-approved option (a)).

> **⚠️ Item 8 / native-as-consumable-substrate is BLOCKED on a design decision (2026-05-15).** Attempting the "real `@graphrefly/native` wrapper" (so a consumer can use native as a substrate) surfaced that the advertised install-time `overrides` drop-in (Q28 = option (c), D198, in `graphrefly-ts` `src/index.ts` + `CLAUDE.md`) is **non-functional and not well-posed**: the napi surface is irreducibly async (Core on tokio blocking pool; sync calls deadlock — D070/D077) while `@graphrefly/graphrefly` consumes pure-ts's **sync** public API. The only coherent path is the **deferred D080** (async-everywhere public API across all siblings; `archive/docs/SESSION-rust-port-architecture.md:1155/1177/1275-1283`), never reconciled with Q28/D198. **User decision: design session first** — see `~/src/graphrefly-ts/archive/docs/SESSION-DS-native-substrate-contract.md` + `~/src/graphrefly-ts/docs/optimizations.md` "Native substrate contract". Until locked, `@graphrefly/native` stays a **parity-test-only arm**; item 8's native-side exposure + the publish stay gated. N1 item-8's TS-side (Impl-contract pin) is already landed and is unaffected.
