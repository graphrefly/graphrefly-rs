...
| **M3 (Slice H — register + set_pausable_mode typed-error promotion)** | `graphrefly-core` | ⏸ scheduled (QA-deferred 2026-05-07) | Promote `register` / `register_state` / `register_producer` / `register_derived` / `register_dynamic` / `register_operator` / `set_pausable_mode` from `assert!` to typed `Result<_, RegisterError>` / `Result<_, SetPausableModeError>`. New error types: `RegisterError::{UnknownDep, OperatorWithoutDeps, InitialOnlyForStateNodes, OperatorSeedSentinel, TerminalDep}` + `SetPausableModeError::WhilePaused`. ~150+ call sites need `.expect()` / `?` propagation. Refcount-discipline for early-return error paths (scratch handle release). Per QA D3=(a). See `porting-deferred.md` "D3 — register / set_pausable_mode typed-error refactor — DEFERRED" for full scope. |
...
| **M3 (Slice E2 — fn ctx + cleanup hooks)** | `graphrefly-core` + binding crates | ⏸ scheduled | R2.4.5 / R2.4.6 / Lock 4.A / Lock 6.D — `{ onRerun, onDeactivation, onInvalidate }` cleanup-hook return type; `ctx.store` mutable bag persisting across deactivation. Crosses cleaving plane — `ctx.store` lives binding-side. Needs `BindingBoundary` extension (a `cleanup_for(NodeId)` hook fired by Core at the right lifecycle points). Schedule blocked on: (a) decision on whether `ctx` extends `FnCtx` or replaces it, (b) binding-side storage shape. |
