...
**M1 closed; M2 closed; M3 Slice A + B + C-1 + C-2 + C-3 + D-substrate landed (2026-05-06).** Slice D-substrate (commit 1 of 2) lands the unified-`register` refactor (D030) — `NodeRecord` drops `kind: NodeKind` field; kind is derived from `(deps.is_empty(), fn_id, op, is_dynamic)` shape via the new `NodeRecord::kind()` method. Public API `Core::kind_of(id) -> Option<NodeKind>` derives via on-call. New `NodeRegistration` / `NodeOpts` / `NodeFnOrOp` types + unified `Core::register(reg) -> NodeId`; existing `register_state/_derived/_dynamic/_operator` are now sugar wrappers (~3 lines each, all delegating to `register()`). New `Core::register_producer(fn_id) -> NodeId` sugar (D031) for producer-shape nodes (zero deps + fn). Producer activation hook in `activate_derived`: producers queue directly into `pending_fires` so the wave engine fires fn once on first subscribe. Producer deactivation hook in `Subscription::Drop`: when last subscriber leaves a producer node, lock-released call to `BindingBoundary::producer_deactivate(node_id)` (new default-no-op trait method, D035). Producer state lives binding-side per the FnCtx-symmetric pattern (D033/D034). 13 new substrate tests in `tests/producer.rs` covering kind derivation, activation lifecycle, deactivation lifecycle, no-double-fire, re-subscribe-after-deactivate, deactivate-doesn't-fire-for-non-producer-kinds, fn-result-emission. **347 tests workspace-wide** (was 334 pre-substrate). `cargo clippy --all-targets -D warnings` clean. `cargo fmt --check` clean. `#![forbid(unsafe_code)]` preserved. **D-ops (zip / concat / race / takeUntil + ProducerCtx + parity scenarios) defer to Commit 2** — the substrate is independently valuable and reviewable.
...
| M3 (Slice D-ops — subscription-managed combinators) | `graphrefly-operators` + parity-tests | 🚧 next | After D-substrate. ProducerCtx in `graphrefly-operators::producer` + zip / concat / race / takeUntil ops + ~20 op tests + parity scenarios. |
