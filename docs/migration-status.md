...
- **✅ D293 — LANDED 2026-05-25 (`/porting-to-rs` patch slice; v0.0.8 candidate).** Closes the "process never exits" hang for every napi consumer (test frameworks, CLI scripts, serverless cold-start, AWS Lambda). Ships as `@graphrefly/native@0.0.8` (patch bump, additive only — no breaking surface). **Two pre-locked-scope shifts simplified the actual landing:** (1) `MailboxOp::Shutdown` variant NOT added — that enum lives in `graphrefly-core/src/mailbox.rs` for cross-thread producer/timer ops, not the `CoreActor` closure-dispatch channel; (2) `closed: AtomicBool` per-method guards NOT added — `actor.run`'s existing `sender.send(...).map_err(...)` branch surfaces the broadened "(actor is shut down or shutting down)" napi error automatically post-shutdown, so Q2a's clear-error contract is met without 15+ per-method guard sites.
  ...
  - **Cross-track-ledger impact:** new §1 row to be added on next sweep — `Impl.close()` widening (cross-arm parity-coupled; both arms ship `close()` per D293).
...
- **D — NEXT `/porting-to-rs` BATCH (3-item bundle, locked 2026-05-25 — same `@graphrefly/native@0.1.0` minor bump; DESIGN SESSION FIRST, then paired-batch implementation).** All three items reshape the napi public surface around honest async + lifecycle semantics, naturally publish together, and need a single design session because the trade-offs cross-cut. **Order:** D293 ships first (patch, additive); THEN D292 design session locks D.1/D.2/D.3 semantics; THEN paired implementation slice mints D295+ (substrate) + D296+ (binding) as v0.1.0.
  - **D.1 — `BenchGraph` arbitrary-fn `g.derived(name, deps, fn)` widening.** TSFN-backed user-fn dispatch on `BenchGraph` mirroring existing `register_user_derived` on `BenchCore`. Lifts the D287 carve-out on `resource-profile.test.ts` test #1 cross-arm. Sub-decisions: closure-cell lifetime, JS-side wrapper integration shape, `Impl` widening surface.
  - **D.2 — Async `BenchBatchContext::commit` + `rollback`.** D291 Case 15a verification surfaced that the locked D288 Path D sync-handle commit deadlocks libuv when TSFN-backed sinks fire in `BatchGuard::Drop`'s success path (sync napi blocks libuv → `bridge_sync_unit` can't reach JS → 3-way deadlock). Convert `commit`/`rollback` to async napi (same shape as D267's `BenchGraph` async-read conversion that closed the analogous deadlock class). Lifts the Case 15a `runIf(pure-ts)` gate. Sub-decisions: reopens D288 Q5 sync-handle premise; per-frame channel ownership semantics; ordering vs `wrapper.js` `closed` flag; whether `Promise.all([rollback, throw])` is still safe shape. **Q2.5 added 2026-05-25 by D291 /qa D3:** panic-propagation contract — sink-panic during `BatchGuard::Drop`'s success-path `fire_deferred` MUST reject the async commit Promise (currently the sync `BatchOp::Commit` arm hangs the caller forever — Blind Hunter BH15). See `porting-deferred.md` § "D291 → D292 carry — `BatchOp::Commit` panic-during-drop hang" for the full bridge entry.
  - **D.3 — `@graphrefly/native` lifecycle / `dispose` → `close` rename + `Symbol.asyncDispose`.** Today's `BenchCore::dispose` only drains subscriptions — the actor worker thread stays alive forever (non-daemon `std::thread::spawn`), so Node won't exit naturally. Every consumer that uses a test framework (vitest/jest), a short-lived script, or serverless cold-start hits the same hang we hit verifying D291. **Permanent fix:** `CoreActor::shutdown(&self)` (sends shutdown op + joins JoinHandle); rename `BenchCore::dispose` → `close`, having it call shutdown after draining subs; wire `Symbol.asyncDispose` on the public `NativeImpl` (and any `Graph`/`Subscription` returns) so `await using impl = createNativeImpl()` works (Node 22+). Optional `process.on('beforeExit')` safety net. New `scenarios/usage/cleanup.test.ts` parity scenario asserting standalone-node exit <100ms post-close + vitest worker exit without `--forceExit`. Sub-decisions: semantics of method calls post-close (error vs noop); whether close cancels in-flight ops or waits; FinalizationRegistry GC fallback yes/no; whether the safety-net hook is opt-in or default-on.
  - **Cross-cutting reason for one design session:** D.2 and D.3 are the same architectural concern at different layers (D.2 = libuv stays free during a single batch frame; D.3 = libuv eventually exits when the consumer is done with the Core). The locked answers should be consistent — e.g. if `close()` cancels in-flight ops, that defines what happens to an unresolved `commit()` Promise; if commit is async-everywhere, `close()` can naturally `await` the actor's clean drain. Don't design these in isolation.
  - **Design session output target:** new `archive/docs/SESSION-DS-native-async-lifecycle.md` (Q1–Q9 lens applied to each of D.1/D.2/D.3 + the cross-cutting Q10 alignment lock). Mint as **D292 (design lock)** in `docs/rust-port-decisions.md`. Then the paired implementation slice mints D293+ (substrate) and D294+ (binding).
