...
> **Historical port-era decision log.** Clean-slate decision authority lives in
> `~/src/graphrefly/decisions/decisions.jsonl`; this file is retained only as an
> audit trail for the retired Rust port model. Do not use it as current
> architecture, parity, or package API guidance.
