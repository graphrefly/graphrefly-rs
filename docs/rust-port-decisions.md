...
- **Rationale / acceptance:** correctness supersedes a bug-inflated speedup — the baseline was fast because broken. The disjoint "speedup" claims are *corrected in the docs*, not the fix reverted. Follow-up **C** (cheap-correct-drain fast-path for trivial disjoint waves: no consumers / no deferred work / no cache snapshots) is scheduled to recover the avoidable fraction (the fn-fire drain cost is largely irreducible — fn fires must drain). The independent value of per-subgraph parallelism in the disjoint regime is **flagged for spec-owner re-validation** against post-D047 numbers, not silently rewritten.
...
- **`/qa` hardening (applied in the same slice):** the owning `BatchGuard::drop` now releases ownership via an RAII `_release_ownership` guard that fires on *every* exit path — normal return, panic-discard `return`, and an unwind through `drain_and_flush()`. This closes a **pre-existing** window (the old `s.in_tick = false` had the identical post-drain placement and was likewise skipped if `drain_and_flush()` unwound) — strictly improved here since a stale slot is now (Core, thread)-local, not Core-global. Remaining leak vector is only `mem::forget(guard)` / process-abort (out of contract; inert key, never false-matches). `/qa` triage: 2 review subagents (Blind + Edge Case Hunter); this was the sole non-trivial finding (BH-major / EC F-1) — fixed now per user direction; EC F-2 (`#[must_use]` on `in_tick()`) + the stale-slot doc accuracy applied; EC F-3 (`Generation` newtype, invariant #8) deferred to the broader newtype sweep (also touches `PARTITION_CACHE`); EC F-4 rejected (verified no lock-discipline defect).
- **Affects:** `graphrefly-core::batch.rs` (`IN_TICK_OWNED` thread_local + helpers; `begin_batch_with_guards`; 3 `commit_emission` reads; `BatchGuard::drop` RAII `_release_ownership`; `#[must_use]` on `in_tick()`; doc comments), `graphrefly-core::node.rs` (`CoreState.in_tick` field + init removed; doc comments). New regression tests: `tests/batch.rs::cross_core_same_thread_batchguard_isolation` (pins /qa F1 under the new keying), `tests/per_subgraph_parallelism.rs::disjoint_partition_concurrent_emits_each_drain_and_deliver` (pins the bug fix: both disjoint threads deliver + no retain leak). Panic→clean-slot covered by existing `tests/batch.rs::batch_panic_discards_pending_wave`. Docs: this entry; `docs/porting-deferred.md` Phase J CORRECTION + Sub-slice-4 `:846` NOTE; `CLAUDE.md` invariant #3 correction; `docs/migration-status.md`.
