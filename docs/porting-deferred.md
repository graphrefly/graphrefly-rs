...
last_updated: 2026-05-25 (D293 LANDED — "process never exits" hang CLOSED for all napi consumers via CoreActor::shutdown + BenchCore::close + Symbol.asyncDispose; two D292 WATCH items recorded below — drop-running-join on libuv thread + silent close-error swallow in wrapper.js. Prior 2026-05-25: D291 Bucket A closure — Case 5 + F2 CI guard CLOSED cross-arm; Case 15a PARTIAL — substrate fix landed but cross-arm lift blocked by libuv-async-commit deadlock; D291 deferred-scope sub-items + D291-async-commit follow-on)
---

## D293 → D292 carries (recorded 2026-05-25, lift in D292's v0.1.0)

D293 closed the "process never exits" hang via the minimum-viable `CoreActor::shutdown` + `BenchCore::close` + `Symbol.asyncDispose` shape. Two adjacent concerns are out of D293's scope but MUST be addressed in D292's lifecycle design session — both surface as in-code `// D292 WATCH:` comments at the relevant sites:

- **Drop-running-join on libuv thread.** `Drop for CoreActor` (and the new `CoreActor::shutdown`) call `handle.join()` synchronously. If `BenchCore` is GC'd on the libuv JS thread (a real path — napi-rs invokes the `napi_class` finalizer on libuv when JS collects the object), `Drop` blocks the JS event loop on `handle.join()` until the worker's current closure completes. For long-running closures (a stuck JS packer callback inside `bridge_sync`, a deep wave drain) this surfaces as a frozen UI / unresponsive Node process — **visually indistinguishable from the very hang D293 is trying to fix**. **Lift shape (D292):** async-shutdown-from-finalizer — `FinalizationRegistry`-driven `napi::bindgen_prelude::spawn` of the shutdown work so finalizers post the join off-thread. **Source:** D293 architecture HALT (2026-05-25); cited in `core_actor.rs::CoreActor::shutdown` doc-comment + `Drop for CoreActor` body.

- **Silent close-error swallow in `wrapper.js`.** `impl.close = async () => { await state.core.close().catch(() => {}); }` swallows actor errors silently for backward-compat with the parity harness's prior `_dispose` shape (afterEach ignored errors). For the new PUBLIC `close()` surface this loses error visibility — a future `close()` failure (network-attached storage tier timeout, async-commit panic propagation, etc.) is hidden from the JS caller. **Lift shape (D292):** lifecycle design session locks whether `impl.close()` rejects on actor errors (cleaner; matches Promise contract) or stays swallow-silent (safer for cleanup paths). Until then this preserves the no-throw cleanup contract every existing caller relies on. **Source:** D293 architecture HALT (2026-05-25); cited in `wrapper.js` `impl.close` block + `BenchCore::close` doc-comment.

**Why deferred (not fixed in D293):** D293 is intentionally minimum-viable — the smaller surface area means consumer-pressure-driven decisions in D292's lifecycle design (FinalizationRegistry / `process.on('beforeExit')` / drain-vs-cancel / async-commit panic propagation) can be made with full context. Lifting either of these in D293 would either widen the surface beyond a patch bump (Symbol.asyncDispose semantic widening = minor bump) or lock semantics prematurely.

---

## Process-never-exits hang — RESOLVED 2026-05-25 (D293)

~~Every napi consumer using a test framework (vitest/jest/mocha), short-lived script, serverless cold-start, or AWS Lambda hit a hang on process exit. Rust's `std::thread::spawn` has no daemon concept on POSIX; the per-`BenchCore` worker thread (`crates/graphrefly-bindings-js/src/core_actor.rs::CoreActor::spawn`) blocks Node's process exit indefinitely until the last `Arc<CoreActor>` drops. Companion classes (`BenchGraph`, `BenchOperators`, `BenchStorage*`, `BenchReactive*`) hold their own Arc clones, so refcount-zero is non-deterministic (JS GC) and often "never" in short-lived consumers. Workarounds: `process.exit(0)` (breaks streaming) or `--forceExit` (jest) / vitest config tricks (`pool: 'threads', poolOptions.threads.isolate: false`). Verified by getting bitten during D291 /qa — couldn't run parity vitest without stubbing rust.ts because of this exact hang.~~ **RESOLVED 2026-05-25 (D293 patch slice; v0.0.8).** Added `CoreActor::shutdown(&self)` (flag + wake closure + take-and-join `JoinHandle`, idempotent); `BenchCore::close()` async napi (drain subs best-effort + actor.shutdown); `BenchCore::dispose()` aliased to `close()` per Q1b. JS surface: `impl.close()` public + `impl[Symbol.asyncDispose] = impl.close` (Node 22+ `await using`). Error message broadened "(actor is shut down or shutting down)" per Q2a so post-close method-call errors are recognizable cross-arm. README "Closing a NativeImpl" documents both `await using` (modern) + `try/finally` (compat) patterns. Cargo regression `d293_shutdown_idempotent_and_post_close_errors_are_clear` + parity scenario `scenarios/usage/cleanup.test.ts` (5 cases). Two D292 carries recorded above. **Canonical:** `~/src/graphrefly-ts/docs/rust-port-decisions.md` D293; cross-track-ledger §1 row to be added on next sweep.

## D291 deferred-scope (2026-05-25, /qa expanded)

D291's Item 1 closed Case 5 (terminal slot + per-dep cascade slot rollback). FOUR terminal-related state mutations in [`Core::terminate_node`](../crates/graphrefly-core/src/node.rs) are NOT yet snapshotted; D291 scoped to the leaf-source + cascade case (which is what Case 5 exercises). Each lift point is independent — pick one when a real consumer surfaces:

- **`rec.has_received_teardown` flag rollback.** [`Core::teardown_inner`](../crates/graphrefly-core/src/node.rs) sets `has_received_teardown = true` on first call (R2.6.4 / Lock 6.F idempotency guard). A `ctx.down(node, [TEARDOWN])` inside a panicking batch leaves the flag set → subsequent `teardown` calls become silent no-ops on the rolled-back node. **Defer-rationale:** the canonical R2.6.4 contract is "TEARDOWN auto-prepends COMPLETE on a non-terminal node"; combined with D291's COMPLETE-slot rollback, a TEARDOWN-mid-batch + throw scenario already restores `rec.terminal = None`, so a re-teardown would re-do the auto-COMPLETE prepend correctly — the `has_received_teardown` flag is only load-bearing for protecting against `[COMPLETE, TEARDOWN]` being delivered twice, which is moot on rollback (no delivery happened). No parity test exercises this gap; lift only if a consumer surfaces. **Lift shape:** new `wave_teardown_snapshots: AHashSet<NodeId>` field on `WaveState` mirroring the `wave_terminal_snapshots` pattern; restore sets `has_received_teardown = false` on panic-discard.

- **`rec.pause_state` buffer drain rollback.** `terminate_node` drains the per-node pause buffer (`rec.pause_state.is_paused()` branch at [`node.rs:4099`](../crates/graphrefly-core/src/node.rs)) and releases the queued payload retains. On rollback, the buffer's content is lost AND the released retains can't be re-claimed. **Defer-rationale (D291 /qa A9 honest re-frame, 2026-05-25):** the pre-/qa rationale framed this as "moot per R1.3.8.b — buffered content is moot post-terminal." That framing is accurate ONLY for the commit path; on the **rollback** path the node is restored to non-terminal, but its pause buffer is gone (handles released, content lost). Subscribers that were paused mid-batch lose buffered DATA / RESOLVED / INVALIDATE waves they would have received on RESUME had the throw never happened. This IS a partial spec-deviation against R1.3.8.b/d's replay semantics — not "moot." Lift gated on a real consumer surfacing a paused-node-mid-batch + throw scenario. **Lift shape:** snapshot `rec.pause_state` (which contains a `Vec<Message>` with retained payload handles); restore swaps the buffer back AND re-retains the handles (or, equivalently, defer the drain to commit-time + drop on panic). Non-trivial because of the refcount math.

- **`rec.replay_buffer` drain rollback.** `terminate_node` unconditionally drains the per-node replay buffer at [`node.rs:4111`](../crates/graphrefly-core/src/node.rs) and releases the handles. On rollback, the buffer is gone. **Defer-rationale:** the cache snapshot restoration (already covered by `wave_cache_snapshots`) preserves the last DATA value; a fresh subscribe post-rollback rebuilds the replay buffer from scratch on the next emit (per R2.6.5 / Slice E1's lazy-fill discipline). Late subscribers would miss DATA values emitted strictly between the pre-batch replay-buffer state and the rollback wave, but no parity test exercises that gap. **Lift shape:** snapshot the SmallVec of handles before drain; restore by re-pushing AND re-retaining each handle. Same refcount-math caveat as pause_state.

- **`ws.pending_wipes.push(id)` queue rollback (D291 /qa D4 — option (b), 2026-05-25).** `terminate_node` pushes onto `ws.pending_wipes` (R2.6.4 wipe queue for resubscribable+no-sub nodes — fires `BindingBoundary::wipe_ctx` at wave-end). On panic-discard the queue is `mem::take`d & dropped silently (`batch.rs:3898`). Pre-D291 this was fine because `rec.terminal` stayed `Some(_)` post-rollback, so the binding's `NodeCtxState` was logically dead anyway. **Post-D291 the rollback restores `rec.terminal = None`**, but the wipe push was already discarded → the binding-side `NodeCtxState` survives indefinitely (D061-style leak class). **Defer-rationale:** D291's locked invariant is "terminal slot + per-dep cascade slot only" (narrow; per the slice's "What this slice does NOT do" lock). Snapshotting `pending_wipes` would widen the invariant without consumer pressure (D196 violation). No parity test exercises a resubscribable-no-sub-node-mid-batch + throw scenario. **Lift shape:** snapshot `ws.pending_wipes.len()` at terminal-mid-batch entry and `truncate()` back on panic-discard restore (similar to a `Vec::split_off(pre_len)` discard). ~10 LOC. Triggers re-look: a real audit-log consumer surfaces, OR a resubscribable-node scenario lands in the parity gate.

**Source:** D291 Bucket A `/porting-to-rs` slice closing-scope decision (2026-05-25); D291 /qa A9 + D4 (2026-05-25); `~/src/graphrefly-ts/docs/rust-port-decisions.md` D291 § "What this slice does NOT do".

## D291 → D292 carry — `BatchOp::Commit` panic-during-drop hang (2026-05-25, /qa D3)

D291 /qa Blind Hunter BH15 / Edge Case Hunter latent finding: in the `BatchOp::Commit` arm of [`run_batch_loop`](../crates/graphrefly-bindings-js/src/batch_bindings.rs) the explicit `drop(guard_holder.take())` runs `BatchGuard::Drop`'s success path → `drain_and_flush` + `fire_deferred`. **If any sink panics during `fire_deferred` (R4.3.5 coalescing), the panic unwinds through `run_batch_loop`, never reaching `reply.send(())` → the JS caller's `rx.recv()` blocks forever.** Pre-D291 the `_handle_guard` would drop on the unwind path (clearing the Q2 tripwire); post-D291 it's already dropped, which is fine for the tripwire but doesn't fix the liveness gap.

**Defer-rationale:** D292 (locked design session for the async `BenchBatchContext::commit` / `rollback` conversion) **rebases this exact surface**. Once `commit` is `async fn` routed through `actor.run`, the core_actor's outer `catch_unwind` (`core_actor.rs:356`) catches the closure panic and converts it to a rejected Promise — the JS caller observes a rejection instead of a hang. **However** the locked D292 scope (Q2.1–Q2.4 in `~/src/graphrefly-ts/docs/optimizations.md` "🔴 DESIGN SESSION FIRST" item) does NOT explicitly include this behavior — it covers Promise contract / closed-flag ordering / per-frame channel ownership / Promise.all-rollback shape. The carry below names BH15 explicitly so D292's design session must lock the panic-propagation contract as a new **Q2.5** sub-decision before the implementation slice starts.

- **What:** `BatchOp::Commit` arm's `drop(guard_holder.take())` is unwind-unsafe — a sink panic on the success path causes the parked actor closure to exit without sending the commit reply, hanging the JS caller forever. Symptom: `await impl.batch(...)` never resolves; `node` process hang on test exit (the same class of bug the D291 verify session ran into, but caused by a sink panic, not the underlying napi worker thread).
- **Why deferred:** D292's async-commit conversion (next `/porting-to-rs` batch) reshapes this code path entirely. Wrapping in `catch_unwind` + widening `Sender<()>` → `Sender<Result<(), String>>` in the current sync shape would be code-thrown-away when D292 lands. Applying within D291 now paints us into the locked sync shape for an extra iteration.
- **Required D292 design-session sub-decision (Q2.5):** lock the panic-propagation contract for the async commit/rollback paths. Specifically: what happens when (a) a sink panics during `BatchGuard::Drop`'s success-path `fire_deferred`, (b) the closure body itself panics post-commit but pre-reply (commit currently can't panic post-`drop(guard_holder.take())`; rollback's `resume_unwind` is the intended path), (c) `actor.run`'s outer `catch_unwind` (`core_actor.rs:356`) is the canonical conversion surface — does the Promise reject with the panic message (M1 currently `let _`s the payload — Rust-dev clarity only per D258 amend), or does it brick the BenchCore? Without an explicit lock, the implementation slice might (i) preserve the M1 "swallow + brick" behavior (regression), (ii) introduce ad-hoc `catch_unwind` at the wrong layer, or (iii) miss the carry entirely.
- **Lift shape (under D292):** `BenchBatchContext::commit` becomes `async fn commit(&self) -> Result<()>` routed through `actor.run(|core| { drop(guard_holder.take()); })`; `actor.run`'s panic-aware `catch_unwind` converts closure panics to `Err(panic_msg)` → JS Promise rejects cleanly. The sync `Sender<()>` is removed entirely (no per-op crossbeam channel needed once `commit`/`rollback` go through `actor.run`'s existing reply machinery).
- **Source:** D291 /qa Blind Hunter BH15 + /qa D3 triage user-direction (2026-05-25 — "Either expand D292's scope explicitly, mint a new D for the catch_unwind widening, or apply within D291 now"). Chose expand-D292: this entry IS the bridge.
