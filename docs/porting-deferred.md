...
> **Historical port-era registry.** This file records deferred concerns from the
> retired port model. Treat entries as audit history unless a current
> `~/src/graphrefly` authority record or [`docs.jsonl`](docs.jsonl) explicitly
> promotes them. It is not current Rust package API guidance.

## D293 → D292 carries — ✅ CLOSED 2026-05-25 (D295 paired-batch implementation slice)

D293 closed the "process never exits" hang via the minimum-viable `CoreActor::shutdown` + `BenchCore::close` + `Symbol.asyncDispose` shape. The two adjacent concerns recorded as carries are now CLOSED by D292's v0.1.0 implementation slice (D295):

- ~~**Drop-running-join on libuv thread.**~~ **RESOLVED 2026-05-25 by D292 D.3 Item 1 (D295 slice).** New `BenchCore::finalize_async()` SYNC `#[napi]` fn posts the shutdown work (subscription drain + `actor.shutdown()`) to tokio's blocking pool via `napi::bindgen_prelude::spawn_blocking`. JS-side `wrapper.js` module-level `_coreFinalizer = new FinalizationRegistry((coreHandle) => coreHandle.finalizeAsync())` + `_coreFinalizer.register(impl, state.core, impl)` at end of `createNativeImpl` (registers off-libuv async shutdown for any NativeImpl that's GC'd without explicit close). The libuv finalizer thread returns immediately; the join happens off-thread. `Drop for CoreActor` stays as the non-libuv safety net (Rust-side ownership transfers). Cargo pin: `d292_finalize_async_off_libuv`.

- ~~**Silent close-error swallow in `wrapper.js`.**~~ **RESOLVED 2026-05-25 by D292 D.3 Item 2 (D295 slice).** `impl.close = state.core.close.bind(state.core)` — dropped the `.catch(() => {})`. Actor errors now surface as rejected Promises per F2 (unified panic-propagation contract across commit / rollback / close — every napi method MUST surface actor panics as rejected Promises with payload-string error message; `wrapper.js` MUST NOT swallow). Users wanting silent cleanup wrap in `try/catch`. F4 dropped the `_dispose` parity-harness alias that motivated the original swallow.

---

## Process-never-exits hang — RESOLVED 2026-05-25 (D293)

~~Every napi consumer using a test framework (vitest/jest/mocha), short-lived script, serverless cold-start, or AWS Lambda hit a hang on process exit. Rust's `std::thread::spawn` has no daemon concept on POSIX; the per-`BenchCore` worker thread (`crates/graphrefly-bindings-js/src/core_actor.rs::CoreActor::spawn`) blocks Node's process exit indefinitely until the last `Arc<CoreActor>` drops. Companion classes (`BenchGraph`, `BenchOperators`, `BenchStorage*`, `BenchReactive*`) hold their own Arc clones, so refcount-zero is non-deterministic (JS GC) and often "never" in short-lived consumers. Workarounds: `process.exit(0)` (breaks streaming) or `--forceExit` (jest) / vitest config tricks (`pool: 'threads', poolOptions.threads.isolate: false`). Verified by getting bitten during D291 /qa — couldn't run parity vitest without stubbing rust.ts because of this exact hang.~~ **RESOLVED 2026-05-25 (D293 patch slice; v0.0.8).** Added `CoreActor::shutdown(&self)` (flag + wake closure + take-and-join `JoinHandle`, idempotent); `BenchCore::close()` async napi (drain subs best-effort + actor.shutdown); `BenchCore::dispose()` aliased to `close()` per Q1b. JS surface: `impl.close()` public + `impl[Symbol.asyncDispose] = impl.close` (Node 22+ `await using`). Error message broadened "(actor is shut down or shutting down)" per Q2a so post-close method-call errors are recognizable cross-arm. README "Closing a NativeImpl" documents both `await using` (modern) + `try/finally` (compat) patterns. Cargo regression `d293_shutdown_idempotent_and_post_close_errors_are_clear` + parity scenario `scenarios/usage/cleanup.test.ts` (5 cases). Two D292 carries recorded above. **Canonical:** `~/src/graphrefly-ts/docs/rust-port-decisions.md` D293 (back-fill entry added 2026-05-25 in the same sweep as the cross-track-ledger row); `~/src/graphrefly-ts/docs/cross-track-ledger.md` §1 D293 row (added 2026-05-25). Row stays open until D292's v0.1.0 lifecycle slice lifts the two D292 WATCH items above.

## D291 deferred-scope (2026-05-25, /qa expanded; D296 reframe 2026-05-26; D297 Item 1 CLOSED + Item 3b ADDED 2026-05-26)

D291's Item 1 closed Case 5 (terminal slot + per-dep cascade slot rollback). FOUR terminal-related state mutations in [`Core::terminate_node`](../crates/graphrefly-core/src/node.rs) were flagged as candidate lift points; D291 scoped to the leaf-source + cascade case (which is what Case 5 exercises). Each remaining lift point is independent — pick one when a real consumer surfaces:

- ~~**`rec.has_received_teardown` flag rollback.**~~ **CLOSED 2026-05-26 (D297).** Pre-implementation HALT premise check inverted the original "academic" framing: post-D291 a retry of `teardown(node)` after a panic-discarded batch was silently no-op'd by [`Core::teardown_inner`](../crates/graphrefly-core/src/node.rs) `Action::Visit`'s `has_received_teardown` idempotency guard, leaving the node in a contradictory state (`terminal=None` ∧ `has_received_teardown=true`) the public surface couldn't reset (`reset_for_fresh_lifecycle` gates on `terminal.is_some()`). The "TEARDOWN auto-prepends COMPLETE on a non-terminal node + re-teardown would re-do the auto-COMPLETE prepend correctly" rationale was wrong — the idempotency guard sits ABOVE the EmitTeardown arm, so re-teardown short-circuits before touching `terminate_node`. **Bilateral convergence:** pure-ts had been conformant since D282 — `_preBatchSnapshot.teardownDone` captured at [`packages/pure-ts/src/core/node.ts:3993`](../../graphrefly-ts/packages/pure-ts/src/core/node.ts) and restored at `:4136`. D297 catches Rust up. **D297 ships:** new `wave_teardown_snapshots: AHashSet<NodeId>` on `WaveState`; snapshot in `teardown_inner` `Action::Visit` (gated on `in_tick`); `Core::restore_wave_teardown_snapshots` resets flag back to `false` on panic-discard; success-path drain extended in `drain_wave_terminal_snapshots`; outermost-wave-start `debug_assert`. 3 cargo regression tests in [`crates/graphrefly-core/tests/d297_teardown_flag_rollback.rs`](../crates/graphrefly-core/tests/d297_teardown_flag_rollback.rs) — retry-teardown-after-panic-succeeds (the regression pin), success-path-preserves-idempotency-flag (R2.6.4 commit-path unchanged), cascade-children-teardown-flags-restore (multi-node panic-rollback). Cross-track-ledger §1 D282 row amended with the tightened conformance claim. **Source:** `~/src/graphrefly-ts/docs/rust-port-decisions.md` D297; `~/src/graphrefly-ts/docs/cross-track-ledger.md` §1 D282 row amendment.

- **`rec.pause_state` buffer drain rollback.** `terminate_node` drains the per-node pause buffer (`rec.pause_state.is_paused()` branch at [`node.rs:4099`](../crates/graphrefly-core/src/node.rs)) and releases the queued payload retains. On rollback, the buffer's content is lost AND the released retains can't be re-claimed. **Defer-rationale (D291 /qa A9 honest re-frame, 2026-05-25):** the pre-/qa rationale framed this as "moot per R1.3.8.b — buffered content is moot post-terminal." That framing is accurate ONLY for the commit path; on the **rollback** path the node is restored to non-terminal, but its pause buffer is gone (handles released, content lost). Subscribers that were paused mid-batch lose buffered DATA / RESOLVED / INVALIDATE waves they would have received on RESUME had the throw never happened. This IS a partial spec-deviation against R1.3.8.b/d's replay semantics — not "moot." **Bilateral convergence note (D297 premise check, 2026-05-26):** pure-ts has been conformant since D282 — `_preBatchSnapshot` includes a pause sub-snapshot at [`packages/pure-ts/src/core/node.ts:4006`](../../graphrefly-ts/packages/pure-ts/src/core/node.ts) per /qa F3, restoring the full pause buffer + lockset on rollback. Rust catch-up gated on a real consumer surfacing a paused-node-mid-batch + throw scenario. **Lift shape:** snapshot `rec.pause_state` (which contains a `Vec<Message>` with retained payload handles); restore swaps the buffer back AND re-retains the handles (or, equivalently, defer the drain to commit-time + drop on panic). Non-trivial because of the refcount math.

- **`rec.replay_buffer` drain rollback.** `terminate_node` unconditionally drains the per-node replay buffer at [`node.rs:4157`](../crates/graphrefly-core/src/node.rs) and releases the handles. On rollback, the buffer is gone. **Defer-rationale (corrected 2026-05-26 by D297 premise check):** the cache snapshot restoration (already covered by `wave_cache_snapshots`) preserves the last DATA value per R4.3.2 atomicity. Per R2.6.5 / Slice E1's lazy-fill discipline, a fresh subscribe post-rollback rebuilds the replay buffer from scratch on the next emit. Late subscribers see handshake = `[START, DATA(restored_cache)]` rather than `[START, ...prior_buffer, DATA(cache)]` — they get the latest DATA (R4.3.2-honest) but lose intermediate history. **Pure-ts is conformant via D282** — `_preBatchSnapshot.replayBuffer` is a shallow content copy at [`packages/pure-ts/src/core/node.ts:3999`](../../graphrefly-ts/packages/pure-ts/src/core/node.ts) (/qa F5 note: "shallow content copy, not length. Buffer at-cap with a mid-batch eviction would otherwise have its head silently lost"). Rust catch-up gated on a real consumer surfacing a late-subscriber-replay-after-rollback scenario. **Lift shape:** snapshot the VecDeque content at batch entry, restore on panic-discard; merges Item 3b's scope. Refcount-math caveat as pause_state.

- **`rec.replay_buffer` push during wave NOT rolled back (Item 3b — NEW 2026-05-26, D297 premise check).** Separate from the terminate-drain case above. `push_replay_buffer` at [`batch.rs:1944`](../crates/graphrefly-core/src/batch.rs) appends to `rec.replay_buffer` DURING the wave (called by `fire_regular` / `commit_emission` for each new emit; evicts head when at cap). On panic-discard, these mid-batch additions stay in the buffer (no snapshot mechanism covers them today). A subscriber that subscribed BEFORE the batch and stays alive across the rollback sees a buffer that contains handles emitted by a wave that didn't actually commit — R4.3.2 atomicity violation. **Pure-ts handles this via the SAME `_preBatchSnapshot.replayBuffer` shallow content copy** (`packages/pure-ts/src/core/node.ts:3999`) — restoring the full pre-batch VecDeque content wipes the mid-batch additions AND restores the eviction-evicted head. **Defer-rationale:** narrow gap; no parity test exercises late-subscriber-replay-after-rollback (the dominant case is the cache snapshot, which IS restored). **Lift shape:** merges into Item 3's scope — the canonical lift is "snapshot the buffer content ONCE at batch entry; restore on panic-discard." Covers BOTH the terminate-drain case (Item 3) AND the push case (Item 3b). Pair with Item 3 when consumer pressure surfaces.

- ~~**`ws.pending_wipes.push(id)` queue rollback (D291 /qa D4 — option (b), 2026-05-25).**~~ **REFRAMED-AND-PINNED 2026-05-26 (D296).** Pre-implementation HALT premise check reframed the original "D061-style leak class" framing: post-D291 the panic-discard's `mem::take` is the CORRECT behavior under R2.4.6 + R4.3.2, not a leak. Walking the path: (1) `restore_wave_terminal_snapshots` resets `rec.terminal = None` — the lifecycle did NOT end. (2) Per **R2.4.6**, `wipe_ctx` fires "on resubscribable terminal RESET" — when a terminal lifecycle COMPLETES, not when one is attempted-and-aborted. (3) Per **R4.3.2** atomicity (post-rollback observable state ≡ pre-batch observable state), the binding's `NodeCtxState` entry is the pre-batch lifecycle that CONTINUES post-rollback; firing `wipe_ctx` here would VIOLATE R4.3.2. The original "lift shape" (snapshot pre-len + `truncate()`) was a behaviorally-identical rename of the existing `mem::take` (since `pending_wipes` is empty at outermost-wave-start by invariant, pre-len is always 0). **D296 ships:** doc-comment hardening at [`discard_wave_cleanup`'s `pending_wipes` drop site](../crates/graphrefly-core/src/batch.rs) citing R2.4.6 + R4.3.2 + 2 regression tests in [`crates/graphrefly-core/tests/d296_pending_wipes_rollback.rs`](../crates/graphrefly-core/tests/d296_pending_wipes_rollback.rs) pinning (a) panic-discard does NOT fire wipe + preserves NodeCtxState, (b) success path fires wipe exactly once. No WaveState widening, no behavior change. **Severity calibration:** dormant on the napi `BenchBinding` (does not override `wipe_ctx`); latent for future bindings (PY/WASM/test bindings) that override `wipe_ctx`. **Source:** `~/src/graphrefly-ts/docs/rust-port-decisions.md` D296.

**Source:** D291 Bucket A `/porting-to-rs` slice closing-scope decision (2026-05-25); D291 /qa A9 + D4 (2026-05-25); D296 HALT premise reframe (2026-05-26); D297 HALT premise reframe + Item 3b discovery (2026-05-26); `~/src/graphrefly-ts/docs/rust-port-decisions.md` D291 § "What this slice does NOT do" + D296 + D297.

## D291 → D292 carry — `BatchOp::Commit` panic-during-drop hang — ✅ CLOSED 2026-05-25 (D295 paired-batch slice; D292 D.2 R3 symmetric `catch_unwind` + R2 `spawn_blocking`)

~~D291 /qa Blind Hunter BH15 / Edge Case Hunter latent finding~~ **RESOLVED 2026-05-25 by D292 D.2 (D295 slice).** The panic-propagation contract Q2.5 added by D291 /qa D3 is locked + implemented:

- **R3 (rollback symmetry):** `BatchOp::Commit`/`Rollback` reply widened from `SyncSender<()>` to `SyncSender<Result<(), String>>`. `run_batch_loop`'s `BatchOp::Commit` arm wraps `drop(guard_holder.take())` in `catch_unwind` + `AssertUnwindSafe` so a sink panic during `BatchGuard::Drop`'s success-path `fire_deferred` (R4.3.5 coalescing) converts to `reply.send(Err(panic_msg))` instead of unwinding through the closure (which would skip the reply and hang the JS caller — BH15).
- **R2 (D255 α-shape correction):** `BenchBatchContext::commit`/`rollback` converted from sync `pub fn` to async `pub async fn` via `tokio::task::spawn_blocking(move || rx.recv())` — NOT `actor.run`, which would self-deadlock by serializing reads behind the pending commit on the single-worker Core actor. `spawn_blocking` keeps the Core actor free; libuv stays free.
- **Cross-arm lift:** Case 15a `runIf(runIfPureTs)` gate dropped in `scenarios/core/batch-throw-rollback.test.ts`. Cargo pin: `d292_async_commit_panic_propagates_as_rejection` + `d292_async_rollback_reply_symmetric_with_commit` (both pass against the 12-test bindings-js gate). Cross-arm parity activation depends on `@graphrefly/native@0.1.0` republish per D265 hold-local.
- **Canonical:** `~/src/graphrefly-ts/docs/rust-port-decisions.md` D295 + D292 (design lock) + cross-track-ledger §1 D282 row (CLOSED 2026-05-25 by D292 D.2).

---

## D290 / D289 follow-ons — CLOSED 2026-05-25 (D291)

- ~~**Case 5 — `BenchBatchContext::rollback` terminal-status restoration gap.**~~ **RESOLVED 2026-05-25 (D291 Item 1).** Added `wave_terminal_snapshots: AHashSet<NodeId>` + `wave_dep_terminal_snapshots: AHashSet<(NodeId, NodeId)>` (D291 /qa D2: re-keyed on dep `NodeId`, not positional dep_idx, to survive mid-batch `set_deps` regenerating `dep_records`) to `WaveState`; `terminate_node` snapshots on `None → Some(_)` transitions (gated on `in_tick`; D291 /qa D1: cascade-block ordering reordered to `snapshot → retain → assign` matching the entry block for panic atomicity); new `Core::restore_wave_terminal_snapshots(s, ws) -> Vec<HandleId>` releases ERROR retains lock-released (re-resolves dep index via `child.dep_index_of(dep_node_id)` per D291 /qa D2); wired into `BatchGuard::discard_wave_cleanup`. 3 cargo regressions in `crates/graphrefly-core/tests/d291_terminal_rollback.rs` (cascade test pins the per-dep slot directly via D291 /qa A7's new `pub(crate) Core::dep_terminal_of(child, dep_node_id)` accessor). Case 5 (`scenarios/core/batch-throw-rollback.test.ts`) now cross-arm green. **Scope note (D291 /qa A8 / F11):** the cross-arm Case 5 lift exercises the THROW path only — `BatchGuard::Drop`'s panic branch runs `discard_wave_cleanup` which does NOT fire sinks, so it bypasses the libuv-async-commit deadlock that blocks Case 15a's commit path. A future variant exercising commit-with-cascade-TSFN-sinks would hit the same deadlock and stay gated until D292's async-commit lift.

- **Case 15a — commit-path Q2 tripwire ordering gap — PARTIAL CLOSE 2026-05-25 (D291 Item 2).** Substrate fix landed: explicit `drop(handle_guard)` BEFORE `drop(guard_holder.take())` in `BatchOp::Commit` arm of `run_batch_loop` so `DURING_BATCH_HANDLE` is clear when `BatchGuard::Drop`'s success path runs `fire_deferred`. Rollback path's decl-order reverse-drop preserved (Q2 invariant on `discard_wave_cleanup` intact). 1 cargo regression `d291_commit_path_clears_during_batch_handle_before_sinks_fire` passes (uses Rust-side `Rc` sinks). **But cross-arm vitest verification surfaced a SEPARATE structural blocker the D288 Path D design didn't anticipate** — `BenchBatchContext::commit` is sync napi → blocks libuv on `rx.recv()` → fire_deferred's TSFN-backed sinks (from JS `src.subscribe(...)`) can't fire because libuv is tied up → **3-way deadlock** between JS commit caller, libuv, and the parked actor. Case 15a stays `runIf(impl.name === "pure-ts")` in `scenarios/core/batch-throw-rollback.test.ts`. **Lift shape:** convert `BenchBatchContext::commit` + `rollback` to **async napi** (same shape as D267's `BenchGraph` read-method conversion that closed the analogous TSFN-deadlock class). That's a substrate-shape rebase that reopens D288 Path D's "sync handle" premise + Q5 sub-decision; queued as a follow-on `/porting-to-rs` slice. **Source:** D291 Bucket A `/porting-to-rs` slice cross-arm verification (2026-05-25); standalone reproducer in `TRASH/_d291-verify.cjs` (Case 5 path completed `cache=6 reemittable`, Case 15a path hung at `await impl.batch`).

- ~~**CI grep guard for `bridge_sync*` tripwire wiring (D289 QA F2).**~~ **RESOLVED 2026-05-25 (D291 Item 4).** New `scripts/check-tripwire-wiring.sh` walks every `fn bridge_sync*` in `crates/graphrefly-bindings-js/src/*.rs` and fails-loud if any body lacks an `assert_no_batch_handle(` call. New `mise run check:tripwire-wiring` task. Wired into `gate.sh` as Step 0 (fast pre-flight before fmt/clippy/nextest).
...
> **Status 2026-05-25:** `rustImpl` is active (`createNativeImpl()` + published `@graphrefly/native`). Text below is the original pre-activation fragility audit — re-validate against current `pnpm test:parity` before treating any item as still open.
...
in-batch because (a) at the time `rustImpl` was still `null` (no observable divergence
...
- **What:** D053 activate-and-triage for `parity-tests/impls/rust.ts`. Adapter now ships as `createNativeImpl()` in `@graphrefly/native` (`wrapper.js`) + `packages/parity-tests/impls/rust.ts`; package published; cross-arm parity active when host `.node` present.
- **Source:** Phase A–C close (2026-05-07); closed with native publish milestone (D203/D206/D207) + ongoing parity widening (D282/D283/etc.).
